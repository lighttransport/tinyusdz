...
| `<asset><material>` (rgba/metallic/roughness/emission) | **Partial** | `UsdShade` Material + `UsdPreviewSurface` under `/World/Materials`, bound to visual geoms via `material:binding`. **Texture maps** in the converted USD (`UsdUVTexture`) are the remaining follow-on |
| `<asset><texture>` (builtin checker/gradient + file) + `<material texture texrepeat>` | **Partial** | the web demo renders them: builtin checker/gradient via canvas, file textures loaded from the asset folder, applied to geom materials with `texrepeat` (the iconic checker ground, robot skins). Converted-USD `UsdUVTexture` emission is the follow-on; skybox is not applied as a scene background |
