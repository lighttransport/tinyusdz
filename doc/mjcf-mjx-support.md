...
| `<asset><texture>` (builtin checker/gradient + file) + `<material texture texrepeat>` | **Partial** | **file textures** become a `UsdUVTexture` (`inputs:file`, `wrapS/T=repeat`, `sourceColorSpace=sRGB`) in the converted USD, with `primvars:st` carried on the meshes (OBJ `vt` loaded). The web demo additionally renders builtin checker/gradient via canvas. Remaining follow-ons: builtin checker/gradient as generated PNG assets in the USD, `texrepeat` tiling (`UsdTransform2d`), skybox-as-background |
