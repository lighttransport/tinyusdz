# TinyUSDZ Next, Next IO, and Tydra Next

This note explains the "next" stack in TinyUSDZ and how it is used by the web
`usdzconvert` pipeline.

## Terminology

TinyUSDZ currently has two USD implementations:
...
`src/next/` is a standalone next-generation TinyUSDZ core. Its main public
header is:
...
int main() {
  ...
  if (!tinyusdz::next::LoadUSDComposed("scene.usdc", &stage, opts, &warn, &err)) {
    // handle err
    return 1;
  }

  if (!tinyusdz::next::WriteUSDC(stage, "flattened.usdc", &err)) {
    // handle err
    return 1;
  }
...
Tydra is TinyUSDZ's USD-to-render-data layer. The legacy Tydra path converts the
legacy `Stage` into `tydra::RenderScene`. Tydra Next does the same job for
`tinyusdz::next::Stage` and lives under:
...
int main() {
  ...
  if (!tinyusdz::next::LoadUSDComposed("scene.usdc", &stage, &warn, &err)) {
    return 1;
  }
  ...
  const tinyusdz::tydra::next::RenderScene& scene = result.scene;
