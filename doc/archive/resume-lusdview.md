# lusdview resume and current work state

> **ARCHIVED — superseded.** The active lusdview roadmap is
> [`../lusdview-tasks.md`](../lusdview-tasks.md); this file retains the detailed
...
UsdPreviewSurface/material correctness work across `lusdview`, its
GL/Vulkan/CUDA/HIP backends, and `lusdrender`. The active roadmap is now
[`doc/lusdview-tasks.md`](../lusdview-tasks.md); this file retains the detailed
...
The new `lusdview-camera-record-equivalence` gate compares default and legacy
...
The focused `lusdview` CTest label passes 16 of 17 registered tests on llvmpipe,
...
backends, and `lusdrender`. Its main behaviors are:
...
- Front/back purpose bindings are carried as `materialId` and
  ...
  `lusdrender` select the appropriate material by face orientation.
...
```text
lusdview-material-binding-inheritance
lusdview-degraded-material
lusdview-backface-material
lusdview-double-sided
lusdview-specular-workflow
lusdview-opacity-material
lusdview_lightrt_bridge_test
next_test_schemas
next_test_schemas_ext
next_test_tydra
examples/lusdview/tests/run-degraded-material.sh
examples/lusdview/tests/run-backface-material.sh
tools/lusdrender/tests/run-degraded-material.sh
tools/lusdrender/tests/run-backface-material.sh
```
...
lusdview_lightrt_bridge_test: passed
lusdview-opacity-material: passed (GL, Vulkan raster/RT, CUDA; HIP unavailable)
lusdview-rt-geomsubset-material: passed (including CUDA next/legacy)
12 focused lusdview/lusdrender material tests: passed
shell syntax and git diff --check: passed
full native CTest after persistent-window hardening: 183/183 passed; 4 optional tests skipped
lusdview-legacy-double-deform: passed, plus 15/15 focused deformation/RT tests
...
The follow-up batch harness starts one long-lived lusdview MCP process, accepts
...
canonical remaining work is in `doc/lusdview-tasks.md`, led by loader-neutral
...
`doc/lusdview-tasks.md` is the single source of truth for active work. The
...
`lusdview-texture-semantic-rt-loader-parity`, covering Vulkan ray query, CUDA,
...
## Prioritized lusdview backlog — historical 2026-07-19 snapshot

This section preserves the earlier completion record. It is not the current
backlog; use `doc/lusdview-tasks.md` for prioritization.
...
- [x] Audit and extend `tests/lusdview/run-usd-assets-batch.sh` and
  `examples/lusdview/tests/usd-assets-goldens.tsv`. The checked-in harness and
  `lusdview-usd-assets-golden` target already exist, so do not create a second
...
failures in the lusdview wrapper/golden CTest. The curated manifest covers the
priority MaterialX, transparency, StandardShaderBall, Teapot, skinning, and
primvar assets. `lusdview-usd-assets-classification` proves the classification
...
including material/node paths and shader IDs. `lusdview` and `lusdrender` feed
...
`lusdview-cli-camera-options` regression covers malformed, zero, non-finite,
...
- [x] Implement deterministic translucent ordering for lusdview raster
...
- [x] Continue converging lusdview and lusdrender on the shared material
  evaluator; compare lusdview GL/Vulkan/RT images with lusdrender `-rtPreview`
...
fed into lusdview's existing displacement path. The same change fixed the
...
Continue optimizing `lusdview` OpenGL first-display performance and peak RSS for
...
- Prefer `build_ninja/` for new native configurations, but the existing
  Release viewer benchmark binary is currently `build/lusdview`.
...
`EPERM` when it tries to spawn the local `lusdcat`, before any parity assertion.
...
Current reproducible lusdview benchmark:

```bash
xvfb-run -a sh -c '/usr/bin/time -v build/lusdview \
  --backend gl \
...
a directly comparable first-useful-frame metric for lusdview rather than using
shutdown time.

## Current measured lusdview state
...
```text
examples/lusdview/main.cc
examples/lusdview/app.cc
examples/lusdview/app.hh
examples/lusdview/scene_loader.hh
examples/lusdview/next_scene_loader.cc
examples/lusdview/gl/gl_renderer.cc
examples/lusdview/gpu_scene.hh
...
```bash
cmake --build build -j16 --target lusdview
...
Also run relevant lusdview render/purpose/wireframe tests if those paths change.
Record at minimum:
...
## Archived `resume.md`: Moana Island lusdrender track
...
```bash
cmake --build build_ninja --target lusdrender -j16
BIN=build_ninja/tools/lusdrender/lusdrender
python3 tools/lusdrender/check_lusdrender_smoke.py "$BIN" . /tmp/lightusd_smoke
python3 tools/lusdrender/bench_island.py --out /tmp/island_bench
```
