...
|---|---|
| `TrickyConnectionToRelocatedAttribute` `/HumanRig/Anim/Face/LEye` | The relocated ATTRIBUTES (`baz`/`foo`) now compose CORRECTLY (composed branch pulls them, `emit_custom` restores `custom`). Sole residual: the CONNECTION TARGET on `.../LEyeRig/rig/Mover.bar.connect = [.../SymEyeRig/Anim.baz]` resolves to the pre-relocate `.../LEyeRig/Anim.baz` instead of `.../LEye.baz`. PRECISE CAUSE: the `bar.connect` opinion is authored in ROOT as an over on the CLASS path `.../SymEyeRig/rig/Mover`, so it reaches the composed Mover as a ROOT IMPLIED-CLASS opinion; its target must chain 3 arcs in ONE longest-prefix `ApplyTarget` — reference (HumanRig→FaceRig), inherit (SymEyeRig→LEyeRig), relocate (LEyeRig/Anim→LEye, authored in the FaceRig stack). Mover itself is NOT relocated. TRIED (reverted, gate-neutral): adding the relocate rename from `m.ReverseApply(src)` in `WithStackRelocates` — it fixes the FaceRig-inherit map but the winning opinion is the ROOT implied-class one, whose map is built by the implied-class propagation (chain loop ~line 687), so the relocate must be chained into THAT map instead. |
