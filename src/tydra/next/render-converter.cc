...
void RenderSceneConverter::AssignMeshMaterialBinding(const Stage& stage,
  ...
  for (const GeomSubset& sub : GetGeomSubsets(mesh_prim)) {
    ...
    if (sub_mat.empty() || mit == scene.material_by_path.end()) continue;
...
bool RenderSceneConverter::ExtractShaderParam(const Stage& stage,
  ...
  if (eval.HasConnection(shader_prim, attr_name)) {
    ...
    for (int hop = 0; hop < 16; ++hop) {
      ...
      if (hop_eval.EvalAssetPath(hop_prim, "inputs:file").has_value() ||
          hop_eval.EvalString(hop_prim, "inputs:file").has_value()) {
        break;  // UsdUVTexture or a MaterialX image node.
      }
      ...
      // MaterialX utility nodes commonly forward their first input rather than
      // the property named by the downstream output connection.
      std::string next_connection;
      if (hop_eval.HasConnection(hop_prim, "inputs:in")) {
        next_connection = hop_eval.GetConnectionPath(hop_prim, "inputs:in");
      } else {
        for (const std::string& name : hop_prim.GetPropertyNames()) {
          if (name.compare(0, 7, "inputs:") == 0 &&
              hop_eval.HasConnection(hop_prim, name)) {
            next_connection = hop_eval.GetConnectionPath(hop_prim, name);
            break;
          }
        }
      }
      if (next_connection.empty()) break;
      connection_path = std::move(next_connection);
    }
