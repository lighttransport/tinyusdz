...
static bool ToGeomMesh(const RenderScene &scene, const RenderMesh &rmesh, GeomMesh *dst, std::vector<GeomSubset> *dst_subsets, std::string *err) {
  ...
  if (has_material) {
    ...
    dst->metas().apiSchemas = new APISchemas(mbAPISchema);
  }
  ...
  for (size_t i = 0; i < scene.meshes.size(); i++) {
    ...
    if ((scene.meshes[i].skel_id > -1) && (size_t(scene.meshes[i].skel_id) < scene.skeletons.size())) {
      ...
      if (!skelMap.count(skel_id)) {
        ...
        skel.metas().apiSchemas = new APISchemas(skelAPISchema);
      ...
      if (mesh.metas().apiSchemas) {
        ...
        mesh.metas().apiSchemas->names.push_back({APISchemas::APIName::SkelBindingAPI, ""});
      } else {
        mesh.metas().apiSchemas = new APISchemas(skelAPISchema);
      }
