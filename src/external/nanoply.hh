...
enum class Format { Ascii, BinaryBigEndian, BinaryLittleEndian };

struct Property {
  std::string name;
  PropType type{PropType::Invalid};
  PropType countType{PropType::Invalid};
  uint32_t offset{0};
  uint32_t stride{0};
...
struct Ply {
  ...
  // when mmapped, point to mmapped address. &data[0] when mmap is off.
  const uint8_t *data_addr{nullptr};

  // data.size()  when mmap is off.
  size_t data_size{0};

  // PLY data(filled when mmaped is off).
  std::vector<uint8_t> data;
};

///
/// We don't provide parse PLY from filename
///

///
/// TODO: Parse PLY with mmaped address
///

///
/// Parse PLY data from memory(buffer).
///
/// @param[in] buf Buffer address
/// @param[in] len Buffer length
/// @param[out] ply Parsed PLY
/// @param[out] err Error message
///
bool parse_ply(const char *buf, const size_t len, Ply *ply, std::string *err);


bool has_property(const Element &elem, const std::string &prop_name);

///
/// Compute fixed-size of Element(a property with list type is not supported)
///
size_t compute_elementSize(const Element &element);

///
/// Returns staring byte address of property 'prop_name'.
///
/// Returns nullptr when specified property name not found.
///
uint8_t *get_property_data(const Element &elem, const std::string &prop_name);

bool get_property_type(const std::string &name, nanoply::PropType &ty);

size_t get_property_size(const PropType &ty);

size_t get_property_size(const std::string &name);

}  // namespace nanoply

#endif  // NANOPLY_HH

#if defined(NANOPLY_IMPLEMENTATION)
...
class Parser {
 public:
  Parser(const StreamReader &sr) : _sr(sr) {}

  bool ParseHeader();

  bool IsAscii() const {
    return (_ply.format == nanoply::Format::Ascii);
  }

  bool ParseAscii();
  bool ParseBinary();

  const Ply &GetPly() const {
    return _ply;
  }
 ...
 private:
  ...
  bool Look(const std::string &tok) {
    if (tok.size() < 1) {
      return false;
    }
    ...
    bool is_same = (memcmp(tok.data(), buf.data(), n) == 0);
  ...
  bool Expect(const std::string &tok) {
    ...
    // assume 'tok' does not include newline
    _cursor_col += n;
  ...
  bool LexFloat(std::string &dst) {
    ...
    bool leading_decimal_dots{false};
    {
      ...
      if ((sc == '+') || (sc == '-')) {
        ...
        PushError("Sign or `.` or 0-9 expected");
      }
    }
    ...
    if ((curr == 'e') || (curr == 'E')) {
      ...
      if ((curr == '+') || (curr == '-')) {
        ...
        PushError("Empty `E' is not allowed");
      }
      ...
      while (!Eof()) {
        ...
        if ((curr >= '0') && (curr <= '9')) {
          ...
          if (has_exp_sign) {
            ...
            PushError("No multiple exponential sign characters");
          }
  ...
  void PushError(const std::string &msg) {
    std::stringstream ss;
    ss << msg << " near line " << std::to_string(_cursor_row + 1) << ", column "
       << std::to_string(_cursor_col + 1) << "\n";
  ...
  size_t _cursor_col{0};
  size_t _cursor_row{0};
  std::string _err;
};

bool Parser::ParseAscii() {
  for (auto &element : _ply.elements) {
...
bool Parser::ParseBinary() {

  for (const auto &element : _ply.elements) {

    bool has_list{false};

    for (const auto &prop : element.properties) {
      if (prop.countType != nanoply::PropType::Invalid) {
        has_list = true;
        break;
      }
    }
    ...
    if (has_list) {
      _err += "TODO: element: " + element.name + "\n";
      return false;
    } else {
      size_t sz = compute_elementSize(element);

      // TODO: mmap
      size_t n = sz * element.count;
      std::vector<uint8_t> buf;
      buf.resize(n);

      if (!_sr.read(n, n, buf.data())) {
        _err += "Failed to read binary data of element: " + element.name + "\n";
      }

      _ply.data.resize(_ply.data.size() + n);
    }
...
bool Parser::ParseHeader() {
  if (!Expect("ply")) {
    _err += "PLY data must start with 'ply'\n";
    return false;
  }

  bool has_format{false};
  std::vector<std::string> &comments = _ply.comments;

  std::vector<nanoply::Element> &elements = _ply.elements;

  if (!SkipWhitespaceAndNewline()) {
    PushError("Failed to parse `ply`:");
    return false;
  }

  while (!Eof()) {
    std::string keyword;
    if (!ReadToken(&keyword)) {
      PushError("Syntax error. Keyword expected:");
      return false;
    }

    if (keyword == "format") {
      // format <format> <version_number>

      if (!SkipWhitespace()) {
        PushError("Failed to parse 'format':");
        return false;
      }

      std::string format;
      if (!ReadToken(&format)) {
        PushError("Failed to read string identifier:");
        return false;
      }

      constexpr auto kAscii = "ascii";
      constexpr auto kBinaryLittleEndian = "binary_little_endian";
      constexpr auto kBinaryBigEndian = "binary_little_endian";

      if (format == kAscii) {
        _ply.format = nanoply::Format::Ascii;
      } else if (format == kBinaryBigEndian) {
        _ply.format = nanoply::Format::BinaryBigEndian;
      } else if (format == kBinaryLittleEndian) {
        _ply.format = nanoply::Format::BinaryLittleEndian;
      } else {
        PushError("Unknown format `" + format + "`:");
        return false;
      }
      ...
      // accept only version '1.0'  for now.
      if (!Expect("1.0")) {
        PushError("version 1.0 expected:");
        return false;
      }
      ...
      if (!Expect(" ")) {
        PushError("Whitespace expected after 'comment' keyword:");
        return false;
      }

      std::string comment;
      if (!ReadUntilNewline(&comment)) {
        PushError("Failed to read comment:");
        return false;
      }
      comments.push_back(comment);
    } else if (keyword == "property") {
      if (!SkipWhitespace()) {
        PushError("Invalid character found after keyword `property`:");
        return false;
      }

      if (elements.empty()) {
        PushError("`element` keyword not described before `property` keyword:");
        return false;
      }

      std::string tok;
      if (!ReadToken(&tok)) {
        PushError("Syntax error. Identifier expected:");
        return false;
      }

      if (!SkipWhitespace()) {
        PushError("Invalid character found:");
        return false;
      }

      nanoply::Property prop;

      if (tok == "list") {
        // <list_num_ty> <list_item_ty> <name>

        std::string num_ty;
        if (!ReadToken(&num_ty)) {
          PushError(
              "Syntax error in `property list` num type. Identifier expected:");
          return false;
        }
        ...
        nanoply::PropType numPropTy;
        if (!get_property_type(num_ty, numPropTy)) {
          PushError("Invalid or unknown property type `" + num_ty + "`:");
          return false;
        }

        std::string item_ty;
        if (!ReadToken(&item_ty)) {
          PushError(
              "Syntax error in `property list` item type. Identifier "
              "expected:");
          return false;
        }
        ...
        nanoply::PropType itemPropTy;
        if (!get_property_type(item_ty, itemPropTy)) {
          PushError("Invalid or unknown property type `" + item_ty + "`:");
          return false;
        }
        ...
        std::string name;
        if (!ReadToken(&name)) {
          PushError("Syntax error in `property` name. Identifier expected:");
          return false;
        }

        prop.type = itemPropTy;
        prop.countType = numPropTy;
        prop.name = name;

      } else {
        nanoply::PropType propTy;
        if (!get_property_type(tok, propTy)) {
          PushError("Invalid or unknown property type `" + tok + "`:");
          return false;
        }
        ...
        prop.type = propTy;
        prop.countType = nanoply::PropType::Invalid;
        prop.name = name;
      }

      if (!SkipWhitespaceAndNewline()) {
        PushError("Failed to parse `property`:");
        return false;
      }
      ...
      if (!SkipWhitespace()) {
        PushError("Invalid character found after keyword `element`:");
        return false;
      }
      ...
      if (!SkipWhitespace()) {
        PushError("Failed to parse 'element':");
        return false;
      }

      std::string sz;
      if (!ReadToken(&sz)) {
        PushError("Failed to read element size:");
        return false;
      }

      int num{0};
      int ret = ParseInt(sz, &num);

      if (ret == 0) {
        // ok
      } else {
        if (ret == -1) {
          PushError("Failed to parse element size. Not a number string:");
        } else if (ret == -2) {
          PushError("Failed to parse element size. Value underflow:");
        } else if (ret == -3) {
          PushError("Failed to parse element size. Value overflow:");
        } else {
          PushError("Failed to parse element size:");
        }
        return false;
      }
      ...
      if (!SkipWhitespaceAndNewline()) {
        PushError("Failed to parse end_header:");
        return false;
      }
      break;
    } else {
      PushError("Unknown keyword `" + keyword + "`");
      return false;
    }
  }

  if (!has_format) {
    PushError("`format` keyword not described");
    return false;
  }

  if (elements.empty()) {
    PushError("No `element` keyword described");
    return false;
  }
...
bool get_property_type(const std::string &name, nanoply::PropType &ty) {
    ...
    constexpr auto kFloat32 = "float32";
    constexpr auto kFloat64 = "float64";
    constexpr auto kDouble = "double";
    constexpr auto kInt8 = "int8";
    constexpr auto kInt16 = "int16";
    constexpr auto kInt32 = "int32";
    constexpr auto kUInt8 = "uint8";
    constexpr auto kUInt16 = "uint16";
    constexpr auto kUInt32 = "uint32";

    if (name == kChar) {
      ty = nanoply::PropType::Char;
    } else if (name == kUChar) {
      ty = nanoply::PropType::UChar;
    } else if (name == kShort) {
      ty = nanoply::PropType::Short;
    } else if (name == kUShort) {
      ty = nanoply::PropType::UShort;
    } else if (name == kInt) {
      ty = nanoply::PropType::Int;
    } else if (name == kUInt) {
      ty = nanoply::PropType::UInt;
    } else if (name == kFloat) {
      ty = nanoply::PropType::Float;
    } else if (name == kFloat32) {
      ty = nanoply::PropType::Float;
    } else if (name == kFloat64) {
      ty = nanoply::PropType::Double;
      ...
      ty = nanoply::PropType::UChar;
    } else if (name == kUInt16) {
      ty = nanoply::PropType::UShort;
    } else if (name == kUInt32) {
      ty = nanoply::PropType::UInt;
    } else {
      ty = nanoply::PropType::Invalid;
    }

    return (ty != nanoply::PropType::Invalid);
  }

  size_t get_property_size(const PropType &ty) {

    size_t sz = 0;
    if (ty == nanoply::PropType::Char) {
      sz = 1;
    } else if (ty == nanoply::PropType::UChar) {
      sz = 1;
    } else if (ty == nanoply::PropType::Short) {
      sz = 2;
    } else if (ty == nanoply::PropType::UShort) {
      sz = 2;
    } else if (ty == nanoply::PropType::Int) {
      sz = 4;
    } else if (ty == nanoply::PropType::UInt) {
      sz = 4;
    } else if (ty == nanoply::PropType::Float) {
      sz = 4;
    } else if (ty == nanoply::PropType::Double) {
      sz = 8;
    } else {
      sz = 0;
    }
  ...
  size_t get_property_size(const std::string &name) {
    nanoply::PropType ty;
    if (get_property_type(name, ty)) {
      return get_property_size(ty);
    }
...
size_t compute_elementSize(const Element &element) {
  ...
  for (const auto &prop : element.properties) {
    if (prop.countType != nanoply::PropType::Invalid) {
      // no list type(dynamically-sizable element) is supported to compute elementSize.
      return 0;
    }
    sum += get_property_size(prop.type);
  }
  return sum;
}

bool parse_ply(const char *buf, const size_t len, Ply *dst_ply, std::string *err)
{
  constexpr auto kPly = "ply";

  if (!dst_ply) {
    return false;
  }

  detail::StreamReader sr(reinterpret_cast<const uint8_t *>(buf), len, /* swap_endian */false); // TODO: endian
  detail::Parser parser(sr);
  if (!parser.ParseHeader()) {
    if (err) {
      (*err) += "Failed to parse PLY header: " + parser.GetError() + "\n";
    }
    return false;
  }

  if (parser.IsAscii()) {
    if (!parser.ParseAscii()) {
    }
  ...
  (*dst_ply) = std::move(parser.GetPly());
