...
#ifdef TINYUSDZ_USE_USDPLY

// Assume implementation is done in external/nanoply.cc
//#define NANOPLY_IMPLEMENTATION
#include "external/nanoply.hh"
...
bool ReadPlyFromFile(const std::string &filepath, tinyusdz::GPrim *prim, std::string *err)
{
#if !defined(TINYUSDZ_USE_USDPLY)
  (void)filepath;
  (void)prim;
  if (err) {
    (*err) = "usdPly is disabled in this build.\n";
  }
  return false;
#else
  ...
  return ReadPlyFromString(str, prim, err);
...
bool ReadPlyFromString(const std::string &str, tinyusdz::GPrim *prim, std::string *err)
{
#if !defined(TINYUSDZ_USE_USDPLY)
  (void)str;
  (void)prim;
  ...
  if ((attrs.vertices.size() % 3) != 0) {
    if (err) {
      (*err) += "[usdPly] Invalid vertices data.\n";
    }
    return false;
  }
