...
# LightUSD Animation Clips Exporter — Blender Addon
#
...
#
# Metadata format (v1):
#   Scalar attributes under userProperties:lightusd:animClips: namespace.
...
#   "userProperties:" prefix is stripped, leaving "lightusd:animClips:*".

bl_info = {
    "name": "LightUSD Animation Clips Exporter",
...
def _write_clip_metadata(stage, clips, fps, concat_offsets=None):
    ...
    if not target or not target.IsValid():
        log.error("[LightUSD] Cannot find any prim to attach metadata")
        return False

    _NS = "userProperties:lightusd:animClips:"
    ...
    # Console output
    print(f"[LightUSD] Wrote {len(clips)} clip(s) to {target.GetPath()}:")
...
class LightUSDAnimClipsPreferences(bpy.types.AddonPreferences):
    bl_idname = __name__
...
class LightUSDAnimClipsHook(bpy.types.USDHook):
    """USD export hook that writes multi-clip metadata to the exported stage."""
    bl_idname = "lightusd_anim_clips"
    bl_label = "LightUSD Anim Clips"
    bl_description = "Write multi-clip animation metadata to USD"

    @staticmethod
    def on_export(export_context):
        if not _HAS_PXR:
            log.warning("[LightUSD] pxr modules not available — skipping hook")
            return True

        stage = export_context.get_stage()
        if not stage:
            log.warning("[LightUSD] No USD stage in export context")
            return False

        clips = gather_animation_clips(bpy.context)
        if len(clips) <= 1:
            if clips:
                log.info("[LightUSD] Only 1 clip — no multi-clip metadata needed")
            return True
...
class EXPORT_OT_lightusd_anim_clips(bpy.types.Operator):
    ...
    """
    bl_idname = "export.lightusd_anim_clips"
    ...
    def execute(self, context):
        ...
        try:
            ...
            log.exception("[LightUSD] Separate file export failed")
            return {'CANCELLED'}
...
def _add_base_reference(clip_path, base_path):
    ...
    try:
        ...
        log.info(f"[LightUSD] Added reference {base_rel} to {clip_path}")

    except Exception as e:
        log.warning(f"[LightUSD] Could not add reference to {clip_path}: {e}")
...
class EXPORT_OT_lightusd_auto_bake(bpy.types.Operator):
    """Concatenate all Actions end-to-end and export as a single USD file."""
    bl_idname = "export.lightusd_auto_bake"
    ...
    def execute(self, context):
        ...
        try:
            ...
            for obj, obj_clips in clips_by_obj.items():
                ...
                temp_track.name = "_lightusd_concat_temp"
            ...
            log.exception("[LightUSD] Auto-bake export failed")
            return {'CANCELLED'}
            ...
            for obj, state in orig_states.items():
                ...
                for track in list(anim_data.nla_tracks):
                    if track.name == "_lightusd_concat_temp":
...
class VIEW3D_PT_lightusd_anim_clips(bpy.types.Panel):
    bl_label = "LightUSD Anim Clips"
    bl_idname = "VIEW3D_PT_lightusd_anim_clips"
...
def menu_func_export(self, context):
    self.layout.operator(
        EXPORT_OT_lightusd_anim_clips.bl_idname,
...
_classes = (
    LightUSDAnimClipsPreferences,
    LightUSDAnimClipsHook,
    EXPORT_OT_lightusd_anim_clips,
    EXPORT_OT_lightusd_auto_bake,
    VIEW3D_PT_lightusd_anim_clips,
)
...
def register():
    ...
    print(f"[LightUSD] Animation Clips Exporter v{bl_info['version']} registered")
