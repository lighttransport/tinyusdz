...
export function installTextureColorTransform(material, mapProperty, transform) {
  ...
  const transforms = material.userData.tinyusdzTextureColorTransforms || {};
  transforms[mapProperty] = transform;
  material.userData.tinyusdzTextureColorTransforms = transforms;
  if (material.userData.tinyusdzTextureColorHookInstalled) return;
  material.userData.tinyusdzTextureColorHookInstalled = true;
  const previousCompile = material.onBeforeCompile?.bind(material);
  const previousCacheKey = material.customProgramCacheKey?.bind(material);
  material.onBeforeCompile = (shader, renderer) => {
    if (previousCompile) previousCompile(shader, renderer);
    const configured = material.userData.tinyusdzTextureColorTransforms || {};
    if (configured.map) {
  ...
  material.customProgramCacheKey = () => {
    ...
    return `${prior}|tinyusdz-cs:${JSON.stringify(
      material.userData.tinyusdzTextureColorTransforms)}`;
  };
}
