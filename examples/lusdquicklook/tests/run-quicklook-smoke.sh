#!/usr/bin/env bash
# lusdquicklook headless smoke test.
#
...
#
# usage: run-quicklook-smoke.sh <lusdquicklook-binary> <repo-root> [outdir]
set -euo pipefail

BIN="${1:?usage: run-quicklook-smoke.sh <binary> <repo-root> [outdir]}"
ROOT="${2:?missing repo root}"
OUT="${3:-${TMPDIR:-/tmp}/lusdquicklook-smoke.$$}"
...
if [ -f "$ROOT/models/suzanne-pbr.usda" ]; then
  ...
  grep -Eq '^\[lusdquicklook\] renderer: (gl|cpu) ' "$OUT/b_report.err" \
...
PY
  ...
  ALPHA="$ROOT/models/lusdquicklook-alpha-modes.usda"
...
PY
  ...
  OVL="$ROOT/models/lusdview-transparency.usda"
