...
bool MakeDrawMesh(const tydra::RenderMesh& mesh, DrawMeshCPU* dmOut) {
  ...
  if (mesh.is_single_indexable) {
    ...
    if (colorFV || colorPerVertex) dm.vertexColors.resize(srcIndices.size() * 3);
...
void BuildDrawInstances(const tydra::RenderScene& rs,
  ...
  for (size_t i = 0; i < rs.instances.size(); ++i) {
    ...
    DrawMeshCPU& dm = out->meshes[static_cast<size_t>(di)];
    if (!touched[static_cast<size_t>(di)]) {
      localBox[static_cast<size_t>(di)] = {dm.aabbMin[0], dm.aabbMin[1],
                                           dm.aabbMin[2], dm.aabbMax[0],
                                           dm.aabbMax[1], dm.aabbMax[2]};
      touched[static_cast<size_t>(di)] = 1;
    }
  ...
  for (size_t di = 0; di < out->meshes.size(); ++di) {
    ...
    const std::array<float, 6>& lb = localBox[di];
    bool first = true;
