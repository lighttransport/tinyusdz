...
void BuildDrawScene(const tydra::RenderScene& rs, DrawScene* out, LoadControl* ctrl) {
  *out = DrawScene{};
  ...
  // --- Mesh world transforms from node hierarchy ---
  std::unordered_map<int, matrix4d> meshXform;
  for (const auto& n : rs.nodes) {
    CollectMeshTransforms(n, &meshXform);
  }

  // --- Textures (dedup by texture_image_id) -> drawTexMap[uvTextureIndex] ---
  std::vector<int> drawTexMap(rs.textures.size(), -1);
  ...
  for (size_t m = 0; m < rs.meshes.size(); ++m) {
    ...
    // Cancellation + render budget (stops before building an over-budget mesh
    // so huge scenes neither freeze the per-frame loop nor thrash VRAM).
    if (ctrl) {
      if (ctrl->cancel.load()) {
        out->truncated = true;
      ...
      const size_t thisTris = srcIndices.size() / 3;
      const size_t estVerts = mesh.is_single_indexable ? nPoints : srcIndices.size();
      const size_t estBytes =
          estVerts * sizeof(DrawVertex) + srcIndices.size() * sizeof(uint32_t);
      if (out->triangleCount + thisTris > ctrl->maxTriangles ||
    ...
    if (xit != meshXform.end()) {
      MatToColMajor(xit->second, world.m);
    }
    std::memcpy(dm.world, world.m, sizeof(world.m));

    // --- World-space AABB from local point bounds (8 corners) ---
    ...
    if (firstBound) {
      for (int c = 0; c < 3; ++c) { out->aabbMin[c] = wmin[c]; out->aabbMax[c] = wmax[c]; }
      firstBound = false;
    } else {
      for (int c = 0; c < 3; ++c) {
        out->aabbMin[c] = std::min(out->aabbMin[c], wmin[c]);
        out->aabbMax[c] = std::max(out->aabbMax[c], wmax[c]);
      }
    }
