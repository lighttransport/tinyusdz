...
# root USD path and it will run tusdview headless with the matching realtime
...
#
# Overrides:
#   OUT_DIR=/tmp/tusdview-large-scenes
#   FRAMES=8
#   TUSDVIEW_EXTRA_ARGS="--rt --mode albedo"
#   TUSDVIEW_SCENE_TIMEOUT=10m
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../../.." && pwd)"
TUSDVIEW="${TUSDVIEW:-$REPO_ROOT/build_ninja/tusdview}"
OUT_DIR="${OUT_DIR:-${TMPDIR:-/tmp}/tusdview-large-scene-profiles}"
FRAMES="${FRAMES:-8}"
TUSDVIEW_EXTRA_ARGS="${TUSDVIEW_EXTRA_ARGS:-}"
TUSDVIEW_SCENE_TIMEOUT="${TUSDVIEW_SCENE_TIMEOUT:-10m}"
SKIP=77
...
run_tusdview_scene() {
  if command -v timeout >/dev/null 2>&1; then
    ...
    timeout --kill-after=15s "$TUSDVIEW_SCENE_TIMEOUT" "$TUSDVIEW" \
      --headless --large-scene-profile "$1" $TUSDVIEW_EXTRA_ARGS \
      --frames "$FRAMES" --screenshot "$2" "$3"
  else
    # shellcheck disable=SC2086 # intentional user-supplied extra CLI flags
    "$TUSDVIEW" --headless --large-scene-profile "$1" \
      $TUSDVIEW_EXTRA_ARGS --frames "$FRAMES" --screenshot "$2" "$3"
  fi
}
...
run_scene() {
  ...
  local img="$base.ppm"
  local log="$base.log"
  echo "=== tusdview large-scene profile: $profile ==="
  echo "asset: $asset"
  echo "log:   $log"
  echo "image: $img"
  ran=1
  ...
  start="$(date +%s)"
  run_tusdview_scene "$profile" "$img" "$asset" >"$log" 2>&1
  local rc=$?
  end="$(date +%s)"
  elapsed=$((end - start))
  bytes=0
