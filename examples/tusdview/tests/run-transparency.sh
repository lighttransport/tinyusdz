#!/usr/bin/env bash
#
# Alpha-blend regression: render two overlapping planes (opaque red behind, a
# 50%-opacity blue in front) and assert the overlap region is BLENDED (shows
# both red and blue), not the opaque-blue that a non-blending forward renderer
# produced. Runs GL and Vulkan raster. SKIPs (77) when no GPU/binary.
#
...
for spec in "gl:--backend gl" "vk:--backend vk"; do
  ...
  # Blended overlap must carry BOTH a red and a blue contribution. Opaque-blue
  # (the pre-fix bug) would give R near 0. Require R>=40 and B>=40.
  if [ "$R" -ge 40 ] && [ "$B" -ge 40 ]; then
    echo "PASS: $tag overlap is alpha-blended"
  else
    echo "FAIL: $tag overlap not blended (expected red+blue, got $R $G $B)"; fail=1
  fi
done
