...
bench="${TUSDVIEW_TEXTURE_BENCH:-build_ninja/tools/tusdview-texture-bench/tusdview_texture_gpu_bench}"
root="${TUSDVIEW_CUDA_HDR_ROOT:-}"
device="${TUSDVIEW_CUDA_DEVICE:-NVIDIA GeForce RTX 5060 Ti}"
report="${TUSDVIEW_CUDA_TEXTURE_REPORT:-/tmp/tusdview-cuda-texture-formats.json}"

if [[ -z "$root" || ! -e "$root" ]]; then
  echo "SKIP: set TUSDVIEW_CUDA_HDR_ROOT to an EXR/HDR file or corpus"
  exit 77
fi
if [[ ! -x "$bench" ]]; then
...
log="$(mktemp "${TMPDIR:-/tmp}/tusdview-cuda-textures.XXXXXX")"
trap 'rm -f "$log"' EXIT
