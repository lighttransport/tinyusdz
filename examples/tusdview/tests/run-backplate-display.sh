...
BIN="${1:?tusdview path required}"
ROOT="${2:?repository root required}"
if [[ "${TUSDVIEW_BACKPLATE_GL_CHILD:-0}" == 1 ]]; then
...
TMP="$(mktemp -d /tmp/tusdview-backplate.XXXXXX)"
trap 'rm -rf "$TMP"' EXIT
...
if [[ -n "${DISPLAY:-}" ]] && xdpyinfo >/dev/null 2>&1; then
  TUSDVIEW_BACKPLATE_GL_CHILD=1 bash "$0" "$BIN" "$ROOT" "$TMP"
else
