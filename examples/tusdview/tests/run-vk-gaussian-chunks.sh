...
half_log="$(timeout 90s "$TUSDVIEW" --next --headless --backend vk --rt \
  --frames 3 --screenshot "$TMP/gaussian-half.ppm" \
  "$ROOT/tests/usda/tusdview-gaussian-half.usda" 2>&1)"
half_rc=$?
...
points_log="$(timeout 90s "$TUSDVIEW" --next --headless --backend vk --rt \
  --frames 3 --screenshot "$TMP/points.ppm" \
  "$ROOT/tests/usda/tusdrender-points.usda" 2>&1)"
points_rc=$?
