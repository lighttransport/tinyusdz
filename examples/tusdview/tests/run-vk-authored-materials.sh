...
DEVICE="${TUSDVIEW_VK_DEVICE:-}"
[ -x "$BIN" ] || { echo "SKIP: tusdview not found at $BIN"; exit "$SKIP"; }
[ -n "$DEVICE" ] || { echo "SKIP: set TUSDVIEW_VK_DEVICE to a discrete GPU"; exit "$SKIP"; }
...
for scene in authored-nested-closures authored-volume-material; do
  asset="$ROOT/examples/tusdview/tests/$scene.usda"
  out="$TMP/$scene.png"
  log="$TMP/$scene.log"
  timeout --kill-after=5s "${TUSDVIEW_AUTHORED_MATERIAL_TIMEOUT:-90s}" \
