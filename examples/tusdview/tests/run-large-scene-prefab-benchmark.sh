...
OUT_DIR="${OUT_DIR:-${TMPDIR:-/tmp}/tusdview-large-scene-prefab}"
PROFILE="${PROFILE:-}"
...
set +e
/usr/bin/time -v "${RUN[@]}" "$TUSDVIEW" "${VIEW_ARGS[@]}" >"$LOG" 2>&1
RC=$?
set -e

cat "$LOG"
if [ "$RC" -ne 0 ]; then
  echo "FAIL: tusdview exited with $RC"
  exit "$RC"
fi
if [ "$GPU_ACCEL" = nvidia ] &&
