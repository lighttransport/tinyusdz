...
device="${TUSDVIEW_NVIDIA_DEVICE:-NVIDIA GeForce RTX 5060 Ti}"
files="${TUSDVIEW_LOCAL_PTEX_FILES:-}"
if [[ -z "$files" ]]; then
  echo "SKIP: set TUSDVIEW_LOCAL_PTEX_FILES to a colon-separated Ptex file list"
  exit 77
fi
if [[ ! -x "$viewer" ]]; then
  echo "FAIL: tusdview executable is missing: $viewer" >&2
  exit 1
fi
...
out="$(mktemp -d "${TMPDIR:-/tmp}/tusdview-ptex-nvidia.XXXXXX")"
if [[ "${TUSDVIEW_LOCAL_KEEP:-0}" == "1" ]]; then
