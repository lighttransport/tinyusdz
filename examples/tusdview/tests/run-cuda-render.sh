...
#
# tusdview's --cuda path loads the CUDA driver API + NVRTC at runtime via cuew
...
# Linux (see doc/tusdview.md). It is run under --headless so no window/X server
# is required.
#
# If CUDA is unavailable (no NVIDIA device/driver, no libcuda/libnvrtc) tusdview
...
#   examples/tusdview/tests/run-cuda-render.sh
# Overrides: TUSDVIEW=<binary>  ASSET=<usd file>
# Also run via ctest: `ctest -R tusdview-cuda-render`.
set -uo pipefail
SKIP=77
...
OUT="$TMP/cuda.ppm"
echo "=== tusdview CUDA pass ==="
log="$("$TUSDVIEW" --headless --cuda --frames 4 --screenshot "$OUT" "$ASSET" 2>&1)"
echo "$log"
