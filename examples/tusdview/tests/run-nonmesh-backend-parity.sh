...
SKIP=77
TMP="$(mktemp -d /tmp/tusdview-nonmesh-parity.XXXXXX)"
xvfb_pid=""
...
scene="$ROOT/tests/usda/tusdview-nonmesh-points-curves.usda"
timeout 60s "$TUSDVIEW" --config "$TMP/config.json" --backend gl --frames 4 \
...
timeout 60s "$TUSDVIEW" --config "$TMP/config.json" --headless --backend vk --frames 4 \
