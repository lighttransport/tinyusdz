...
run_case() {
  ...
  log="$(timeout 30s "$TUSDVIEW" --headless --backend vk --frames 1 "$asset" 2>&1)"
  rc=$?
  if [ "$rc" -ne 0 ]; then
    ...
    echo "$log"
    echo "FAIL: tusdview failed for $asset"
    exit 1
  fi
...
opacity_log="$(timeout 30s "$TUSDVIEW" --headless --backend vk --frames 1 \
