...
void Store3(const float src[3], float* dst) {
  dst[0] = src[0];
  dst[1] = src[1];
  dst[2] = src[2];
}
...
void PackLightRtOpenPBR(const DrawMaterialCPU& mat, float* dst) {
  ...
  Store3(m.baseColor, dst + 0);            dst[3] = m.baseWeight;
  Store3(m.specularColor, dst + 4);        dst[7] = m.specularWeight;
  Store3(m.transmissionColor, dst + 8);    dst[11] = m.transmission;
  Store3(m.transmissionScatter, dst + 12); dst[15] = m.transmissionDepth;
  Store3(m.subsurfaceColor, dst + 16);     dst[19] = m.subsurface;
  Store3(m.subsurfaceRadius, dst + 20);    dst[23] = m.subsurfaceScale;
  Store3(m.coatColor, dst + 24);           dst[27] = m.coatWeight;
  Store3(m.sheenColor, dst + 28);          dst[31] = m.sheenWeight;
  Store3(m.emissionColor, dst + 32);       dst[35] = m.emission;
  Store3(m.normal, dst + 36);              dst[39] = m.opacity;
  dst[40] = m.metalness;
  dst[41] = m.diffuseRoughness;
  dst[42] = m.specularRoughness;
  dst[43] = m.specularIor;
  dst[44] = m.coatRoughness;
  dst[45] = m.coatIor;
  dst[46] = m.sheenRoughness;
  dst[47] = m.transmissionScatterAnisotropy;
  dst[48] = m.thinFilmWeight;
  dst[49] = m.thinFilmThicknessNm;
  dst[50] = m.thinFilmIor;
  dst[51] = mat.hasLightRtOpenPBR ? 1.0f : 0.0f;
  dst[52] = m.hasTextureInputs ? 1.0f : 0.0f;
  dst[53] = m.hasNormalInput ? 1.0f : 0.0f;
  dst[54] = static_cast<float>(mat.alphaMode);
  dst[55] = mat.alphaCutoff;
}
