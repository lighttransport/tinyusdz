...
#if defined(_WIN32)
  if (auto base = PathFromEnv("APPDATA")) return *base / "lusdview" / "config.json";
  if (auto base = PathFromEnv("LOCALAPPDATA")) return *base / "lusdview" / "config.json";
  if (auto base = PathFromEnv("USERPROFILE")) return *base / "lusdview" / "config.json";
  return std::nullopt;
#elif defined(__APPLE__)
  if (auto home = PathFromEnv("HOME")) {
    return *home / "Library" / "Application Support" / "lusdview" / "config.json";
  }
  return std::nullopt;
#else
  if (auto base = PathFromEnv("XDG_CONFIG_HOME")) return *base / "lusdview" / "config.json";
  if (auto home = PathFromEnv("HOME")) return *home / ".config" / "lusdview" / "config.json";
  return std::nullopt;
#endif
}
