...
// lusdview - wrapper around the vendored texture tools (src/external/textools:
// tir resize, texcomp block compression, texpipe mip pipeline, envmap IBL).
//
// This is the only lusdview header/TU pair that touches the textools C API, so
// the rest of the app stays independent of LIGHTUSD_WITH_TEXTOOLS. Every entry
// point returns false (leaving outputs untouched) when lusdview is built
// without LUSDVIEW_WITH_TEXTOOLS; callers fall back to the legacy paths.
#pragma once
