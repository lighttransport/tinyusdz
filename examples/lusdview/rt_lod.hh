...
// lusdview - view-dependent LOD selection for the interactive Vulkan ray-query
// renderer. Pure math (Vulkan-free, light3d only) so it is unit-testable and can
// be reused by lusdrender. Given the camera and each prototype's instances, it
...
struct RtLodCamera {
  ...
  bool prototypeLodEnabled{false};
  float prototypeErrorPx{0.0f};  // 0 requests authored LOD0
