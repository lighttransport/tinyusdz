...
#
# lusdview's --hip path loads the HIP runtime + hiprtc at runtime via hipew (no
...
#
# If HIP is unavailable (no AMD/ROCm device, no libamdhip64/libhiprtc) lusdview
...
#   examples/lusdview/tests/run-hip-render.sh
# Overrides: LUSDVIEW=<binary>  ASSET=<usd file>
# Also run via ctest: `ctest -R lusdview-hip-render`.
set -uo pipefail
SKIP=77
...
OUT="$TMP/hip.ppm"
echo "=== lusdview HIP pass ==="
log="$("$LUSDVIEW" --headless --hip --frames 4 --screenshot "$OUT" "$ASSET" 2>&1)"
echo "$log"
