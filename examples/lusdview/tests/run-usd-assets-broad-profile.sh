...
if LUSDVIEW="$FAKE" LUSDVIEW_FAKE_ARGS="$ARGS" LUSDVIEW_XVFB=0 \
   USD_ASSETS_ROOT="$ROOT" \
   bash "$SCRIPT_DIR/run-usd-assets-render-smoke.sh" \
...
LUSDVIEW_XVFB=0 \
USD_ASSETS_ROOT="$ROOT" \
LUSDVIEW_USD_ASSETS_ANIM_MODES=vk-raster \
LUSDVIEW_USD_ASSETS_ANIM_OUT="$TMP/animation" \
