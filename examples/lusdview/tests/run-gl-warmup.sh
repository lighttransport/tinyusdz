...
# shards launch lusdview. The full material fragment program can take about a
...
OUT="$(mktemp -d /tmp/lusdview-gl-warmup.XXXXXX)"
trap 'rm -rf "$OUT"' EXIT
...
LOG="$(timeout --kill-after=5s "${LUSDVIEW_GL_WARMUP_TIMEOUT:-150s}" \
  "${RUN[@]}" "$BIN" --backend gl --frames 1 --size 96x96 --no-grid \
  --screenshot "$OUT/warmup.ppm" \
  "$ROOT/tests/usda/lusdview-raster-multilight-links.usda" 2>&1)"
rc=$?
if [ "$rc" -ne 0 ]; then
  echo "$LOG"
...
if [ "${LUSDVIEW_NVIDIA_OFFLOAD:-0}" = "1" ] &&
