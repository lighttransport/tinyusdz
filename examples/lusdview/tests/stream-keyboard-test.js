...
//
// End-to-end browser test for the lusdview WebSocket stream viewer's keyboard
...
//
// Usage:
//   node stream-keyboard-test.js [--lusdview ./build/lusdview]
...
const opt = {
  // ctest passes the built binary path via the LUSDVIEW env var.
  lusdview: process.env.LUSDVIEW || arg('lusdview', './build/lusdview'),
...
// Spawn a headless-VK lusdview stream server and resolve once it is listening.
function startServer() {
  return new Promise((resolve, reject) => {
    // NOTE: --stream-http takes its value with '=' (an optional-value flag).
    const p = spawn(opt.lusdview, ['--headless', '--backend', 'vk',
    ...
    p.on('exit', c => { if (!done) reject(new Error('lusdview exited early (code ' + c + ')')); });
...
(async () => {
  ...
  if (!opt.url) {
    ...
    catch (e) { console.log('SKIP: lusdview stream server did not start (' + e.message + ').'); process.exit(SKIP); }
  }
