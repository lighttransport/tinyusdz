...
TMP="$(mktemp -d /tmp/lusdview-gl-vk-parity.XXXXXX)"
trap 'rm -rf "$TMP"' EXIT
...
run_bounded() {
  if command -v timeout >/dev/null 2>&1; then
    timeout --kill-after=5s "${LUSDVIEW_GL_VK_TIMEOUT:-20s}" "$@"
  else
    "$@"
  fi
}
...
FIXTURES=(
  "$ROOT/tests/usda/lusdview-raster-multilight-links.usda"
  "$ROOT/tests/usda/lusdview-shadow-alpha-inst.usda"
)
