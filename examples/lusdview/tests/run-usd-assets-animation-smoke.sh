...
OUT="${LUSDVIEW_USD_ASSETS_ANIM_OUT:-}"
MODES="${LUSDVIEW_USD_ASSETS_ANIM_MODES:-vk-raster,lusdr-cpu}"
TIME_A="${LUSDVIEW_USD_ASSETS_TIME_A:-0}"
TIME_B="${LUSDVIEW_USD_ASSETS_TIME_B:-1}"
SKIP=77
...
if [ -n "${LUSDVIEW_USD_ASSETS_GATE:-}" ] &&
   [ -z "${!LUSDVIEW_USD_ASSETS_GATE:-}" ]; then
  echo "SKIP: gate variable '$LUSDVIEW_USD_ASSETS_GATE' is not set"
  exit "$SKIP"
fi
...
run_time() {
  local time="$1" out="$2"
  LUSDVIEW_USD_ASSETS_FILES="$FILES" \
