...
int main() {
  ...
  {
    lusdview::DrawMaterialCPU noiseMaterial;
  ...
  {
    lusdview::DrawMaterialCPU subsurfaceMaterial;
    ...
    if (!lusdview::CompileMaterialXGraphRuntime(&subsurfaceMaterial,
                                                &subsurfaceError) ||
  ...
  const lusdview::DrawMaterialCPU& mat = draw.materials[0];
  ...
  const auto hasSamplingIntent = [](const lusdview::DrawTexSampleCPU& sample,
                                    lusdview::DrawColorSpace colorSpace) {
    return sample.wrapS == lusdview::WrapMode::Repeat &&
           sample.wrapT == lusdview::WrapMode::Mirror &&
           sample.colorSpace == colorSpace;
  };
  if (!hasSamplingIntent(mat.baseColorSample, lusdview::DrawColorSpace::sRGB) ||
      !hasSamplingIntent(mat.metallicSample, lusdview::DrawColorSpace::Raw) ||
      !hasSamplingIntent(mat.roughnessSample, lusdview::DrawColorSpace::Raw) ||
      !hasSamplingIntent(mat.normalSample, lusdview::DrawColorSpace::Raw) ||
      !hasSamplingIntent(mat.coatNormalSample, lusdview::DrawColorSpace::Raw) ||
      !hasSamplingIntent(mat.emissiveSample, lusdview::DrawColorSpace::sRGB) ||
      !hasSamplingIntent(mat.opacitySample, lusdview::DrawColorSpace::Raw) ||
      !hasSamplingIntent(mat.coatWeightSample, lusdview::DrawColorSpace::Raw) ||
      !hasSamplingIntent(mat.coatColorSample, lusdview::DrawColorSpace::sRGB) ||
      !hasSamplingIntent(mat.coatRoughnessSample, lusdview::DrawColorSpace::Raw) ||
      !hasSamplingIntent(mat.specularColorSample, lusdview::DrawColorSpace::sRGB)) {
    std::fprintf(stderr,
  ...
  for (const lusdview::DrawMaterialParamCPU& param : mat.params) {
  ...
  const lusdview::LoadDiagnostics diagnostics =
      lusdview::CategorizeLoadWarnings("", draw.skipped);
  ...
  lusdview::DrawMaterialCPU liveMat = mat;
  float connectedBaseParam = -1.0f;
  for (const lusdview::DrawMaterialParamCPU& param : liveMat.params) {
  ...
  lusdview::DrawLightRtOpenPBRCPU live = liveMat.lightRtOpenPBR;
  ...
  live.opacity = 0.65f;
  lusdview::ApplyOpenPBRMaterialConstants(&liveMat, live);
  if (!Near(liveMat.lightRtOpenPBR.baseWeight, 0.37f) ||
      ...
      liveMat.alphaMode != static_cast<int>(lusdview::AlphaMode::Blend)) {
  ...
  lusdview::DrawMaterialCPU constantMat = mat;
  constantMat.hasUsdPreviewSurface = true;
  lusdview::MakeConstantOpenPBRMaterial(&constantMat);
  ...
  for (const lusdview::DrawMaterialParamCPU& param : constantMat.params) {
  ...
  const lusdview::DrawMaterialCPU& graphMat = draw.materials[1];
  ...
  if (!graphMat.materialXGraph.valid ||
      ...
      graphMat.materialXGraph.nodes[0].op !=
          lusdview::MaterialXGraphOpCPU::Multiply) {
    std::fprintf(stderr,
