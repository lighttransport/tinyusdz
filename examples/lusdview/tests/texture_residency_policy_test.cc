...
void TestPriorityOrder() {
  using lusdview::TextureDemandPriority;
  ...
  const std::vector<size_t> ordered =
      lusdview::OrderTextureDecodeCandidates(priorities, eligible);
...
void TestEvictionGatesAndLru() {
  using lusdview::TextureEvictionCandidate;
  ...
  const size_t victim = lusdview::ChooseTextureEvictionVictim(
...
void TestEvictionTieAndNoVictim() {
  ...
  CHECK(lusdview::ChooseTextureEvictionVictim(tied, 2, 2.0) == 1);
  const TextureEvictionCandidate pinned[] = {
      {0, 5.0, 1, true, false}, {1, 1.0, 1, false, false}};
  CHECK(lusdview::ChooseTextureEvictionVictim(pinned, 2, 2.0) == 2);
}
