...
NVIDIA_ENV=()
VK_PREFIX=()
if [ "${LUSDVIEW_NVIDIA_OFFLOAD:-0}" = "1" ]; then
  NVIDIA_ENV=(
  ...
  export "${NVIDIA_ENV[@]}"
  if [ "${LUSDVIEW_XVFB:-1}" = "1" ] && command -v xvfb-run >/dev/null 2>&1; then
...
timeout --kill-after=5s "${LUSDVIEW_OPENCHESS_TIMEOUT:-180s}" \
  env -u LUSDVIEW_VK_DEVICE "$BIN" --headless --backend vk --cpu-rt \
...
run_cuda="${LUSDVIEW_OPENCHESS_CUDA:-auto}"
...
# the leg with LUSDVIEW_OPENCHESS_VKRT=1 (or suppress it with =0).
run_vkrt="${LUSDVIEW_OPENCHESS_VKRT:-auto}"
...
if [ "$run_vkrt" = "1" ]; then
  timeout --kill-after=5s "${LUSDVIEW_OPENCHESS_VKRT_TIMEOUT:-180s}" \
...
timeout --kill-after=5s "${LUSDVIEW_OPENCHESS_RASTER_TIMEOUT:-120s}" \
  "${VK_PREFIX[@]}" "$BIN" --headless --backend vk \
  "${VK_DEVICE_ARGS[@]}" \
  --frames 1 --size 128x128 \
...
if command -v Xvfb >/dev/null 2>&1; then
  ...
  if [ -n "${LUSDVIEW_OPENCHESS_GL_DISPLAY:-}" ]; then
    GL_DISPLAY="$LUSDVIEW_OPENCHESS_GL_DISPLAY"
  ...
  if [ -n "$GL_DISPLAY" ] && kill -0 "$XVFB_PID" >/dev/null 2>&1; then
    timeout --kill-after=5s "${LUSDVIEW_OPENCHESS_GL_TIMEOUT:-120s}" \
