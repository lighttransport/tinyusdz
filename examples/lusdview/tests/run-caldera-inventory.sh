...
OUT_DIR="${OUT_DIR:-/tmp/lusdview-caldera-inventory}"
LUSDCAT="${LUSDCAT:-${ROOT_DIR}/build_ninja/lusdcat}"
JOBS="${JOBS:-8}"
TIMEOUT_SECS="${TIMEOUT_SECS:-30}"

if [[ ! -x "${LUSDCAT}" ]]; then
  echo "FAIL: lusdcat not executable: ${LUSDCAT}" >&2
  exit 1
fi
...
if [[ "${1:-}" == --probe ]]; then
  ...
  timeout "${TIMEOUT_SECS}s" /usr/bin/time -f '__TIME__ %e %M %x' \
    "${LUSDCAT}" -l --memstat "${scene}" >"${record}.out" 2>&1
  rc=$?
  set -e
