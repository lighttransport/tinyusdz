 ...
 */
#ifndef LIGHTTYPE_UTF8_UTIL_H
#define LIGHTTYPE_UTF8_UTIL_H

/*
 * Return byte length of the UTF-8 codepoint starting at buf[pos].
 * Returns 0 if pos >= len.  Returns 1 for invalid lead bytes.
 */
static inline int ltt__utf8_cp_len(const char *buf, int pos, int len)
{
    if (pos >= len) return 0;
 ...
 */
static inline int ltt__utf8_prev(const char *buf, int pos)
{
    if (pos <= 0) return 0;
    pos--;
