#!/usr/bin/env bash
# tusdquicklook INTERACTIVE smoke test (Xvfb).
#
...
#
# usage: run-quicklook-gui-smoke.sh <tusdquicklook-binary> <repo-root>
set -euo pipefail
...
echo "== tusdquicklook GUI smoke on $DISP"
...
WID="$(DISPLAY="$DISP" xwininfo -root -children 2>/dev/null \
       | grep tusdquicklook | awk '{print $1}' || true)"
[ -n "$WID" ] || fail "no tusdquicklook window was mapped (window-create hang?)"
...
echo "== tusdquicklook GUI smoke: PASS"
