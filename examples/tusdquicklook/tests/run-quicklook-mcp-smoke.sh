#!/usr/bin/env bash
# tusdquicklook MCP stdio smoke test.
#
...
echo "== tusdquicklook MCP stdio smoke"
set +e
printf '%s\n' \
...
grep -q 'tusdquicklook-mcp' "$OUT/stdout" || fail "server identity missing"
...
echo "== tusdquicklook MCP stdio smoke: PASS"
