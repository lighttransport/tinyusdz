...
"""TinyUSDZ — a lightweight USD (Universal Scene Description) library.

Built on the tinyusdz "next" core: load USDA/USDC/USDZ (composed or
layer-level), inspect and author stages, and extract render-ready geometry
via the ``tinyusdz.tydra`` submodule.
...
def asarray(array: Array):
    """Wrap a tinyusdz.Array in a numpy.ndarray without copying.
