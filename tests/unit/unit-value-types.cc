...
void role_type_cast_test(void) {
  ...
  // Test 1: Scalar float2 -> texcoord2f
  {
    ...
    const void* ptr_before = val.as<value::float2>();
    ...
    const void* ptr_after = val.as<value::texcoord2f>();
  ...
  // Test 2: Scalar float3 -> normal3f
  {
    ...
    const void* ptr_before = val.as<value::float3>();
    ...
    const void* ptr_after = val.as<value::normal3f>();
    TEST_CHECK(ptr_before == ptr_after);
  ...
  // Test 7: Array float2[] -> texcoord2f[]
  {
    ...
    const void* ptr_before = val.as<std::vector<value::float2>>();
    ...
    const void* ptr_after = val.as<std::vector<value::texcoord2f>>();
    TEST_CHECK(ptr_before == ptr_after);
  ...
  // Test 8: Array float3[] -> normal3f[]
  {
    ...
    const void* ptr_before = val.as<std::vector<value::float3>>();

    bool result = RoleTypeCast(value::TypeTraits<std::vector<value::normal3f>>::type_id(), val);
    TEST_CHECK(result == true);

    const void* ptr_after = val.as<std::vector<value::normal3f>>();
    TEST_CHECK(ptr_before == ptr_after);
