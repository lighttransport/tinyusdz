...
TUSD="${TUSDCHECKER_PATH:-build/tusdchecker}"
if [ ! -x "$TUSD" ]; then
  echo "ERROR: tusdchecker not found at $TUSD (set TUSDCHECKER_PATH)"
  exit 2
fi

FIXTURE_DIR="${FIXTURE_DIR:-tests/usda}"

# pxr "<Validator>.<Rule>" -> space-separated acceptable tusd rule ids.
# A pxr finding is satisfied when ANY of the mapped ids appears in the
# tusdchecker report for the same fixture.
map_rule() {
  case "$1" in
...
for f in "$FIXTURE_DIR"/*.usda; do
  ...
  while IFS= read -r rule; do
    ...
    hit=0
    for id in $mapped; do
      if printf '%s' "$tusd_out" | grep -qF "[$id]"; then hit=1; break; fi
    done
    if [ "$hit" -eq 0 ]; then
...
if [ "$unmapped_count" -gt 0 ]; then
  status=1
  echo ""
  echo "UNMAPPED pxr rules (no tusd rule id mapping in this script):"
...
if [ "$unmatched_count" -gt 0 ]; then
  status=1
  echo ""
  echo "UNMATCHED findings (mapped, but tusdchecker did not report any mapped id):"
...
if [ "$status" -eq 0 ]; then
  echo "PARITY OK: every pxr finding family has a mapped tusdchecker finding."
else
  echo ""
