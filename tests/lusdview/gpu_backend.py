"""Is lusdview's renderer a software rasterizer?
...
has DRI) lusdview's mesh draw fetches only `aPosition` correctly: every other
...
lusdview prints its device at startup, e.g.
  [lusdview] renderer: OpenGL, GPU: Mesa llvmpipe (LLVM 20.1.8, 256 bits), ...
  [lusdview] renderer: Vulkan, GPU: AMD Radeon RX 9070 XT (RADV GFX1201), ...
"""
...
def vk_device_args(backend):
    """Extra lusdview args selecting the hardware Vulkan device, when present.
    ...
    selection routes the backend to the hardware device (doc/lusdview.md,
    "Vulkan on NVIDIA PRIME/offload under Xvfb"). lusdview's --vk-device does
...
def gpu_offload_env(base=None):
    ...
    context to the hardware device anyway (doc/lusdview.md, "Headless
...
def is_software_renderer(viewer_output):
    """True when lusdview's startup banner names a software device."""
...
def device_name(viewer_output):
    """The `GPU: ...` field of lusdview's banner (for log messages).
...
def software_only_vulkan():
    ...
    """
    vulkaninfo = shutil.which("vulkaninfo")
    def no_visible_linux_gpu():
        return (os.name == "posix" and os.path.isdir("/dev") and
                not os.path.exists("/dev/nvidia0") and
                not glob.glob("/dev/dri/renderD*"))

    if not vulkaninfo:
        return no_visible_linux_gpu()
    try:
        r = subprocess.run(
            [vulkaninfo, "--summary"], stdout=subprocess.PIPE,
            stderr=subprocess.STDOUT, timeout=10, check=False)
    ...
    if r.returncode != 0:
        # Headless sandboxes commonly have a host kernel module/ICD but expose
        # no render node. vulkaninfo then fails while lusdview falls back to
        # llvmpipe; treat that as software-only instead of running minute-long,
        # non-representative ray-query comparisons.
        return no_visible_linux_gpu()
