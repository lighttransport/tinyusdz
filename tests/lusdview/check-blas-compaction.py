#!/usr/bin/env python3
"""lusdview Vulkan RT: BLAS compaction must shrink the acceleration structures.

lusdview built one BLAS per prototype at its *build-time* size and kept it
...
compacted size, and copy into right-sized storage. This asserts:
  ...
  2. it is IMAGE-NEUTRAL -- the compacted render is byte-identical to the
     uncompacted one (LUSDVIEW_BLAS_COMPACT=0).
...
def render(binary, scene, out, work, compact):
    try:
        ...
        pass
    env = dict(os.environ)
    env["LUSDVIEW_BLAS_COMPACT"] = "1" if compact else "0"
...
def main():
    if len(sys.argv) < 4:
        print("usage: check-blas-compaction.py <lusdview> <model> <work_dir>")
        return SKIP
    ...
    if ("ray tracing is unavailable" in on_log or
            (not re.search(r"caps: v1 .*rt=hardware", on_log) and
             "[vk_rt]" not in on_log)):
