#!/usr/bin/env python3
"""lusdview: the large-scene budget tree must descend from the REAL GPU.
...
def run(binary, scene, work, extra):
    ...
    if "[lusdview][error] --" in text:
        print("FAIL: lusdview rejected an argument from this test -- the test "
...
def main():
    if len(sys.argv) < 3:
        print("usage: check-vram-budget.py <lusdview> <work_dir>")
        return SKIP
