#!/usr/bin/env python3
#
# Compare lusdview CPU and GPU skinning screenshots.
#
...
def parse_args():
    ...
    parser.add_argument("--app", required=True, help="Path to the lusdview executable.")
    ...
    parser.add_argument("--mode", default=None, help="Forward --mode NAME to lusdview.")
    parser.add_argument("--vk-device", default=None, help="Forward --vk-device INDEX|NAME to lusdview.")
    parser.add_argument("--headless", action="store_true", help="Use lusdview's Vulkan headless path.")
    parser.add_argument(
        "--skip-unavailable",
        action="store_true",
    ...
    parser.add_argument(
        ...
        default=[],
        metavar="NAME=VALUE",
        help="Environment variable to set for lusdview; may be repeated.",
    )
    parser.add_argument(
        "--nvidia-offload",
        action="store_true",
...
def main():
    ...
    if not app.is_file():
        return fail(f"lusdview executable not found: {app}")
    ...
    print(
        "lusdview CPU/GPU skinning screenshot diff: "
