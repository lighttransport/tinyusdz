#!/usr/bin/env python3
"""tusdrender `-autoframe` regression: a FLAT scene must not frame edge-on.
...
Usage: check_autoframe_flat.py <tusdrender> <repo_root> <work_dir>
...
def main():
    if len(sys.argv) < 4:
        print("usage: check_autoframe_flat.py <tusdrender> <repo_root> <work_dir>")
        return SKIP
