#!/usr/bin/env bash
#
# Regression test: tusdrender's tydra-next (-rtPreview) path must sample the UV
...
R() {
  "$TUSDRENDER" "$1" "$2" -rtPreview -autoframe -w 128 -height 128 -samples 2 \
...
if a==b:
    print("FAIL: uvSet1 and st renders are byte-identical -- tusdrender ignored "
