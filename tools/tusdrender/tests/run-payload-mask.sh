#!/usr/bin/env bash
# Verify that tusdrender's next-loader mask is also a payload composition filter.
set -uo pipefail

TUSDRENDER="${1:?usage: $0 /path/to/tusdrender}"
ROOT="$(cd "$(dirname "$0")/../../.." && pwd)"
SCENE="$ROOT/tests/feat/large-scene/fixture/deferred-nested/root.usda"
TMP="$(mktemp -d /tmp/tusdrender-payload-mask.XXXXXX)"
trap 'rm -rf "$TMP"' EXIT

if ! timeout 30s "$TUSDRENDER" "$SCENE" "$TMP/selected.png" \
...
set +e
timeout 30s "$TUSDRENDER" "$SCENE" "$TMP/deferred.png" \
