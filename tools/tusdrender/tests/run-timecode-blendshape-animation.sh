#!/usr/bin/env bash
#
# tusdrender next-loader blendshape regression. The red foreground mesh has a
# BlendShape target driven by SkelAnimation blendShapeWeights; frame 2 moves it
# away from center and reveals a blue background.
set -uo pipefail
...
TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT
ASSET="$TMP/timecode_blendshape.usda"
OUT="$TMP/frame.####.png"
LOG="$TMP/timecode_blendshape.log"
...
def Xform "World" {
  ...
  def Mesh "RedFront" (
    ...
    rel skel:blendShapeTargets = </World/RedFront/Slide>
    rel material:binding = </World/Mats/Red>

    def BlendShape "Slide" {
      point3f[] offsets = [(3,0,0),(3,0,0),(3,0,0),(3,0,0)]
    }
  }
  def Scope "Mats" {
    def Material "Red" {
...
if [ "$R1" -gt 150 ] && [ "$B1" -lt 80 ] && \
  ...
  echo "PASS: blendshape weights change rendered frame color"
  exit 0
fi
