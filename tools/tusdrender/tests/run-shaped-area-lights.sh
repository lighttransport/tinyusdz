#!/usr/bin/env bash
#
# tusdrender: RectLight / DiskLight / CylinderLight must be AREA lights.
#
...
#      (TUSDR_LIGHT_NEE=0), and fewer hard steps; and
...
render() {  # $1 = scene, $2 = out, $3 = TUSDR_LIGHT_NEE, $4.. = extra flags
  local scene="$1" out="$2" nee="$3"
  shift 3
  TUSDR_LIGHT_NEE="$nee" "$TUSDRENDER" "$scene" "$out" -rtPreview \
    -viewDir 0,0,1 -bg 0,0,0 -ambient 0 \
    -materialShading lightrt-bsdf -materialResolver tydra-next "$@" \
    > "$out.log" 2>&1
  if [ ! -s "$out" ]; then
    echo "FAIL: tusdrender failed for $out"
    cat "$out.log"
    return 1
  fi
  return 0
