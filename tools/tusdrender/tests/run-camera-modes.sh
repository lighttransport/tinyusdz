#!/usr/bin/env bash
#
# tusdrender RT-preview camera regression. Two authored cameras look at two
...
render_camera() {
  ...
  "$TUSDRENDER" "$ASSET" "$out" -rtPreview -camera "$cam" \
    -w 64 -height 64 -samples 1 -ambient 1 -noShadows -stats \
    >"$log" 2>&1
  local rc=$?
  if [ "$rc" -ne 0 ]; then
    echo "FAIL: tusdrender $cam exited with $rc"
    cat "$log"
    exit 1
  fi
  if [ ! -s "$out" ]; then
    echo "FAIL: tusdrender $cam produced no image"
    cat "$log"
    exit 1
  fi
...
if [ "$RR" -gt 100 ] && [ "$RR" -gt $((RB * 3)) ] &&
  ...
  echo "PASS: tusdrender authored camera selection changes the view"
  exit 0
fi
