#!/usr/bin/env bash
#
# tusdrender RT-preview opacity regression. Translucent blue and green planes sit
...
ASSET="${2:-${ASSET:-$REPO_ROOT/models/tusdview-transparency.usda}}"
...
render_case() {
  ...
  OUT="$TMP/${label}.png"
  LOG="$TMP/${label}.log"
  "$TUSDRENDER" "$asset" "$OUT" -rtPreview -w 96 -height 96 -autoframe \
    -viewDir 0,0,-1 -ambient 0.2 -samples 1 >"$LOG" 2>&1
  rc=$?
  if [ "$rc" -ne 0 ]; then
    echo "FAIL: tusdrender exited with $rc for $label"
    cat "$LOG"
    exit 1
  fi
  if [ ! -s "$OUT" ]; then
    echo "FAIL: tusdrender produced no image for $label"
    cat "$LOG"
    exit 1
  fi
...
echo "PASS: tusdrender sorted opacity blend preserves layer order"
exit 0
