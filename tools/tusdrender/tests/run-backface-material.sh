...
BIN="${1:-${TUSDRENDER:-$REPO_ROOT/build_ninja/tools/tusdrender/tusdrender}}"
[ -x "$BIN" ] || { echo "SKIP: tusdrender not found ($BIN)"; exit "$SKIP"; }
TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT
...
echo "PASS: tusdrender selects the authored degraded back-face material"
