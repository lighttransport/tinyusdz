#!/usr/bin/env bash
#
# Optional local smoke/performance harness for tusdrender large-scene profiles.
#
...
#
# Overrides:
#   OUT_DIR=/tmp/tusdrender-large-scenes
#   WIDTH=960 HEIGHT=540
#   TUSDRENDER_EXTRA_ARGS="-stats -gpuShade preview"
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../../.." && pwd)"
TUSDRENDER="${TUSDRENDER:-$REPO_ROOT/build_ninja/tools/tusdrender/tusdrender}"
OUT_DIR="${OUT_DIR:-${TMPDIR:-/tmp}/tusdrender-large-scene-profiles}"
WIDTH="${WIDTH:-960}"
HEIGHT="${HEIGHT:-540}"
TUSDRENDER_EXTRA_ARGS="${TUSDRENDER_EXTRA_ARGS:-}"
SKIP=77

if [ ! -x "$TUSDRENDER" ]; then
  echo "SKIP: tusdrender binary not found at $TUSDRENDER (set TUSDRENDER=...)"
  exit "$SKIP"
fi
...
run_scene() {
  ...
  local img="$base.png"
  local log="$base.log"
  echo "=== tusdrender large-scene profile: $profile ==="
  echo "asset: $asset"
  echo "log:   $log"
  echo "image: $img"
  ran=1
  ...
  "$TUSDRENDER" "$asset" "$img" -largeSceneProfile "$profile" \
    $TUSDRENDER_EXTRA_ARGS -w "$WIDTH" -height "$HEIGHT" >"$log" 2>&1
  local rc=$?
  end="$(date +%s)"
  elapsed=$((end - start))
  bytes=0
...
echo "PASS: tusdrender large-scene profile smoke runs completed"
