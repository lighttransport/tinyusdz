#!/usr/bin/env bash
#
# Renderer-facing subdivision coverage: tusdrender's legacy render path must
...
run_case() {
  ...
  log="$("$TUSDRENDER" "$asset" "$out" -w 48 -height 48 -subdiv "$level" -stats 2>&1)"
  echo "$log"
...
render_grid() {
  ...
  "$TUSDRENDER" "$asset" "$TMP/${label}.png" -w 96 -height 96 -subdiv 2 \
...
compare_grid_images() {
  python3 - "$REPO_ROOT/examples/tusdview/tests" "$TMP/grid_plain.png" "$TMP/grid_crease.png" <<'PY'
import sys
