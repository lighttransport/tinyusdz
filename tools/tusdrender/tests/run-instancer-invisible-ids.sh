...
TUSDRENDER="${1:-${TUSDRENDER:-$REPO_ROOT/build_ninja/tools/tusdrender/tusdrender}}"
...
run_case() {
  ...
  local log
  log="$("$TUSDRENDER" "$asset" "$out" -rtPreview -stats -w 64 -height 64 \
  ...
  if [ ! -s "$out" ]; then
    echo "FAIL: $label tusdrender did not write an output image"
    exit 1
  fi
