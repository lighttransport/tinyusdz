...
bool BuildGpuTriScene(const std::vector<Vec3> &base_colors,
  ...
  return BuildGpuCpuScene(threads, out);
}
...
bool ShadeAndWriteImageInstanced(const Options &opt, const GpuInstancedScene &s,
  ...
  const unsigned nthreads =
      std::min<unsigned>(WorkerThreadCount(opt.threads), h > 0 ? unsigned(h) : 1u);
  if (nthreads <= 1) {
    shade_rows(0, h);
  } else {
    std::vector<std::thread> pool;
    pool.reserve(nthreads);
    const int rows_per = (h + int(nthreads) - 1) / int(nthreads);
    for (unsigned t = 0; t < nthreads; ++t) {
      const int y0 = int(t) * rows_per;
      const int y1 = std::min(h, y0 + rows_per);
      if (y0 >= y1) break;
      pool.emplace_back(shade_rows, y0, y1);
    }
