...
plugin_binary="$script_dir/Plugins/TinyUSDZGroomExport/Binaries/Linux/libUnrealEditor-TinyUSDZGroomExport.so"
if [ ! -f "$plugin_binary" ] || find "$script_dir/Plugins/TinyUSDZGroomExport/Source" -type f -newer "$plugin_binary" -print -quit | grep -q .; then
  "$ue_root/Engine/Build/BatchFiles/Linux/Build.sh" UnrealEditor Linux Development \
    -Project="$script_dir/TinyUSDZMetaHuman.uproject" -WaitMutex
fi
...
extra_args=()
if [ "${TINYUSDZ_UE_AUTORIG:-0}" = "1" ]; then
  extra_args+=( -TinyUSDZAutoRig )
fi
