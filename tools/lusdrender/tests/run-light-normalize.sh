...
# lusdrender ignored the flag entirely -- and worse, a SphereLight was ALWAYS
# implicitly normalized by pi*r^2, a convention matching neither UsdLux, the
# other shapes, nor lusdview.
#
...
check_pair() {
  ...
  "$LUSDRENDER" "$a" "$TMP/a.png" -w 64 -height 64 -samples 4 -autoframe "$@" \
      >/dev/null 2>&1 || fail "$label: render A failed"
  "$LUSDRENDER" "$b" "$TMP/b.png" -w 64 -height 64 -samples 4 -autoframe "$@" \
...
"$LUSDRENDER" "$TMP/sphere_raw.usda" "$TMP/raw.png" -w 64 -height 64 -samples 4 \
    -autoframe -rtPreview >/dev/null 2>&1 || fail "unnormalized render failed"
"$LUSDRENDER" "$TMP/sphere_norm.usda" "$TMP/norm.png" -w 64 -height 64 -samples 4 \
