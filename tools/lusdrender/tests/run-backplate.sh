...
LUSDRENDER="${1:?lusdrender path required}"
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
TMP="$(mktemp -d /tmp/lusdrender-backplate.XXXXXX)"
trap 'rm -rf "$TMP"' EXIT
...
"$LUSDRENDER" "$VISIBLE" "$TMP/visible.png" "${COMMON[@]}"
"$LUSDRENDER" "$HIDDEN" "$TMP/hidden.png" "${COMMON[@]}"
"$LUSDRENDER" "$BEHIND" "$TMP/behind.png" "${COMMON[@]}"
"$LUSDRENDER" "$SINGLE" "$TMP/single.png" "${COMMON[@]}"
