...
ASSET="$REPO_ROOT/tests/usda/lusdview-nonmesh-points-curves.usda"
...
TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT
LUSDR_HERMITE_SEGMENTS=8 "$LUSDRENDER" "$ASSET" "$TMP/curves-cpu.png" \
...
LUSDR_HERMITE_SEGMENTS=8 "$LUSDRENDER" "$ASSET" "$TMP/curves.png" -vkr -w 32 -height 32 \
