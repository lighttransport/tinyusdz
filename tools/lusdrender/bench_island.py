#!/usr/bin/env python3
"""Benchmark lusdrender vs. Pixar usdrecord (hdEmbree) on Moana Island elements.

For each independent per-element geometry USD file this runs both renderers under
``/usr/bin/time -v`` (peak RSS + wall) and, for lusdrender, parses the ``-stats``
...
``tools/lusdrender/README.md`` and ``doc/large-scene.md``.
...
def run_lusdrender(binpath, scene, out_png, width, height, extra, timeout):
    cmd = [
    ...
    stats = parse_lightusd_stats(err)
...
def main():
    ...
    ap.add_argument("--bin", default=os.path.join(
        REPO, "build_ninja", "tools", "lusdrender", "lusdrender"),
        help="lusdrender binary")
    ...
    ap.add_argument("--lightusd-extra", default="",
                    help="extra args passed to lusdrender as one string, "
                         "shlex-split (e.g. --lightusd-extra='-maxMem 4')")
    ap.add_argument("--skip-usdrecord", action="store_true")
    ap.add_argument("--skip-lusdrender", action="store_true")
    args = ap.parse_args()
    ...
    for name in args.elements:
        ...
        if not args.skip_lusdrender:
            out_png = os.path.join(args.out, f"{name}_tusd.png")
            print("  lusdrender ...", file=sys.stderr)
            r = run_lusdrender(args.bin, scene, out_png, args.width,
                               args.height, shlex.split(args.lightusd_extra),
                               args.timeout)
            row["lightusd"] = r
            st = r["stats"]
    ...
    # Markdown table
    md = []
    md.append(f"| element | size | tris | lightusd load s | lightusd bvh s | "
              f"lightusd render s | lightusd total s | lightusd RSS | usdrecord s | "
    ...
    for row in rows:
        ...
        u = row.get("usdr", {})
        st = t.get("stats", {})
        lightusd_total = t.get("wall_s")
        usdr_total = u.get("wall_s")
        speed = "-"
        if isinstance(lightusd_total, (int, float)) and isinstance(
                usdr_total, (int, float)) and lightusd_total > 0:
            speed = f"{usdr_total / lightusd_total:.1f}x"
        tris = st.get("triangles")
        tris_s = f"{tris/1e6:.2f} M" if tris else "-"
        lightusd_status = "" if t.get("ok", True) else (
        ...
        md.append(
            ...
            f"{fmt_s(st.get('render_s'))} | {fmt_s(lightusd_total)}{lightusd_status} | "
