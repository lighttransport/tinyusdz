#!/usr/bin/env python3
"""Generate/check TinyUSDZ's supported-feature OpenUSD parity contract.

The OpenUSD schema manifest is descriptive upstream data. This ledger adds the
product decision about which domains TinyUSDZ promises to preserve, compose,
...
EXCLUDED_DOMAINS = {
    "usdHydra": "Hydra/imaging integration is outside TinyUSDZ's renderer-neutral core",
    ...
    if args.check:
        ...
        if actual != expected:
            print(
                ...
                "scripts/generate-tinyusdz-capability-ledger.py",
                file=sys.stderr)
            return 1
        payload = json.loads(actual)
        print(
            "TinyUSDZ/OpenUSD capability contract: "
