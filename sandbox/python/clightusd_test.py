...
class Token(object):
    ...
    def __init__(self, tok: str):
        #print("__init__")
        self._handle = clightusd.c_lightusd_token_new(tok)
    ...
    def __del__(self):
        ret = clightusd.c_lightusd_token_free(self._handle)
        assert ret == 1

    def __str__(self):
        btok = clightusd.c_lightusd_token_str(self._handle)
        return btok.decode()
...
class String(object):
    ...
    def __init__(self, s: str):
        ...
        self._handle = clightusd.c_lightusd_string_new(s)
...
class Value(object):
    _np_conv_table = {
        "float32": [clightusd.c_lightusd_value_new_float, clightusd.c_lightusd_value_new_array_float]
    }
    ...
    def __init__(self, value):
        ...
        if isinstance(value, np.ndarray):
            if str(value.dtype) in self._np_conv_table:
                ...
                if value.ndim == 0:
                    # scalar
                    self._handle = clightusd.c_lightusd_value_new_float(value.ctypes.data_as(ctypes.c_float))
                elif value.ndim == 1:
                    # 1D array

                    c_type = ctypes.POINTER(ctypes.c_float)

                    #self._handle = clightusd.c_lightusd_value_new_array_float(value.size, value.ctypes.data_as(ctypes.POINTER(ctypes.c_float)))
                    self._handle = clightusd.c_lightusd_value_new_array_float(value.size, value.ctypes.data_as(c_type))
                else:
            ...
            if str(value.dtype) in self._np_conv_table:
                ...
                self._handle = clightusd.c_lightusd_value_new_float(np.ctypeslib.as_ctypes(value))
            else:
                raise RuntimeError("Unsupported dtype {}".format(value.dtype))
    ...
    def __del__(self):
        if self._handle is not None:
            ret = clightusd.c_lightusd_value_free(self._handle)
            assert ret == 1

    def __str__(self):
        if self._handle is not None:
            cstrbuf = clightusd.c_lightusd_string_new_empty()
            ret = clightusd.c_lightusd_value_to_string(self._handle, cstrbuf)
            assert ret == 1

            cstr = clightusd.c_lightusd_string_str(cstrbuf)

            s = cstr.decode() # str

            clightusd.c_lightusd_string_free(cstrbuf)
...
class Prim(object):
    ...
    def __init__(self, prim_type:str = "Model", name: Optional[str] = None, from_handle = None):
        ...
        if from_handle is not None:
            ...
            assert name == None
            self._prim_type = clightusd.c_lightusd_prim_type(self._handle)
            self._name = clightusd.c_lightusd_prim_element_name()
            ...
            err = clightusd.c_lightusd_string_new_empty()
            self._handle = clightusd.c_lightusd_prim_new(prim_type, err)

            if self._handle is False:
                raise RuntimeError("Failed to new Prim:" + clightusd.c_lightusd_string_str(err))

            clightusd.c_lightusd_string_free(err)
    ...
    def __del__(self):
        if not self._is_handle_reference:
            ret = clightusd.c_lightusd_prim_free(self._handle)
            print("del", ret)
    ...
    def children(self):
        ...
        n = clightusd.c_lightusd_prim_num_children(self._handle)
        for i in range(n):
            child_ptr = ctypes.POINTER(clightusd.CLightUSDPrim)()

            ret = clightusd.c_lightusd_prim_get_child(self._handle, i, ctypes.byref(child_ptr))
            assert ret
    ...
    def add_child(self, child_prim):
        ...
        ret = clightusd.c_lightusd_prim_append_child(self._handle, child_prim._handle)
...
class PrimChildIterator:
    def __init__(self, handle):
        self._handle = handle
        self._num_children = clightusd.c_lightusd_prim_num_children(self._handle)
    ...
    def __next__(self):
        if self._current_index < self._num_children:
            # Just a reference.
            child_ptr = ctypes.POINTER(clightusd.CLightUSDPrim)
            ret = clightusd.c_lightusd_prim_get_child(self._handle, ctypes.byref(child_ptr))
            assert ret == 1
