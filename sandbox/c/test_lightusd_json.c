...
static int test_json_object_operations() {
    ...
    lightusd_json_value_t *new_val = lightusd_json_value_create_string("replaced");
    TEST_ASSERT(lightusd_json_object_set(obj, "key1", new_val), "Failed to replace key1");
    TEST_ASSERT(lightusd_json_object_size(obj) == 3, "Object size should still be 3");

    lightusd_json_value_t *replaced_val = lightusd_json_object_get(obj, "key1");
    TEST_ASSERT(replaced_val == new_val, "Replaced value mismatch");
    TEST_ASSERT(strcmp(lightusd_json_value_get_string(replaced_val), "replaced") == 0, "Replaced value incorrect");
...
static int test_json_parser_basic() {
    ...
    lightusd_json_value_t *exp_val = lightusd_json_parse("1.23e-4");
    ...
    TEST_ASSERT(exp_val != NULL && lightusd_json_value_is_number(exp_val), "Failed to parse exponential");
    ...
    TEST_ASSERT(lightusd_json_value_get_number(exp_val) == 1.23e-4, "Exponential value incorrect");

    lightusd_json_value_destroy(int_val);
    lightusd_json_value_destroy(float_val);
    lightusd_json_value_destroy(neg_val);
    lightusd_json_value_destroy(exp_val);
...
static int test_usd_value_json_conversion() {
    ...
    lightusd_value_t *bool_usd = lightusd_value_create_bool(1);
    lightusd_json_value_t *bool_json = lightusd_value_to_json(bool_usd);
    TEST_ASSERT(bool_json != NULL && lightusd_json_value_is_bool(bool_json), "Bool USD->JSON conversion failed");
    TEST_ASSERT(lightusd_json_value_get_bool(bool_json) == 1, "Bool JSON value incorrect");

    struct lightusd_value_t *bool_usd_back = lightusd_json_to_value(bool_json);
    TEST_ASSERT(bool_usd_back != NULL && bool_usd_back->type == LIGHTUSD_VALUE_BOOL, "Bool JSON->USD conversion failed");
    TEST_ASSERT(bool_usd_back->data.bool_val == 1, "Bool USD value incorrect");

    lightusd_value_destroy(bool_usd);
    lightusd_json_value_destroy(bool_json);
    lightusd_value_destroy(bool_usd_back);

    /* Test int conversion */
    lightusd_value_t *int_usd = lightusd_value_create_int(42);
    lightusd_json_value_t *int_json = lightusd_value_to_json(int_usd);
    TEST_ASSERT(int_json != NULL && lightusd_json_value_is_number(int_json), "Int USD->JSON conversion failed");
    TEST_ASSERT(lightusd_json_value_get_number(int_json) == 42.0, "Int JSON value incorrect");

    struct lightusd_value_t *int_usd_back = lightusd_json_to_value(int_json);
    TEST_ASSERT(int_usd_back != NULL && int_usd_back->type == LIGHTUSD_VALUE_INT, "Int JSON->USD conversion failed");
    TEST_ASSERT(int_usd_back->data.int_val == 42, "Int USD value incorrect");

    lightusd_value_destroy(int_usd);
    lightusd_json_value_destroy(int_json);
    lightusd_value_destroy(int_usd_back);

    /* Test double conversion */
    lightusd_value_t *double_usd = lightusd_value_create_double(3.14159);
    lightusd_json_value_t *double_json = lightusd_value_to_json(double_usd);
    TEST_ASSERT(double_json != NULL && lightusd_json_value_is_number(double_json), "Double USD->JSON conversion failed");
    TEST_ASSERT(lightusd_json_value_get_number(double_json) == 3.14159, "Double JSON value incorrect");

    struct lightusd_value_t *double_usd_back = lightusd_json_to_value(double_json);
    TEST_ASSERT(double_usd_back != NULL && double_usd_back->type == LIGHTUSD_VALUE_DOUBLE, "Double JSON->USD conversion failed");
    TEST_ASSERT(double_usd_back->data.double_val == 3.14159, "Double USD value incorrect");

    lightusd_value_destroy(double_usd);
    lightusd_json_value_destroy(double_json);
    lightusd_value_destroy(double_usd_back);

    /* Test string conversion */
    lightusd_value_t *string_usd = lightusd_value_create_string("Hello, USD!");
    lightusd_json_value_t *string_json = lightusd_value_to_json(string_usd);
    TEST_ASSERT(string_json != NULL && lightusd_json_value_is_string(string_json), "String USD->JSON conversion failed");
    TEST_ASSERT(strcmp(lightusd_json_value_get_string(string_json), "Hello, USD!") == 0, "String JSON value incorrect");

    struct lightusd_value_t *string_usd_back = lightusd_json_to_value(string_json);
    TEST_ASSERT(string_usd_back != NULL && string_usd_back->type == LIGHTUSD_VALUE_STRING, "String JSON->USD conversion failed");
    TEST_ASSERT(strcmp(string_usd_back->data.string_val, "Hello, USD!") == 0, "String USD value incorrect");

    lightusd_value_destroy(string_usd);
    lightusd_json_value_destroy(string_json);
    lightusd_value_destroy(string_usd_back);
...
static int test_usd_property_json_conversion() {
    ...
    /* Create a property */
    lightusd_property_t *prop = lightusd_property_create("testProp", "float", LIGHTUSD_PROP_ATTRIB);
    TEST_ASSERT(prop != NULL, "Failed to create property");

    /* Set property attributes */
    lightusd_property_set_custom(prop, 1);
    lightusd_property_set_variability(prop, LIGHTUSD_VARIABILITY_UNIFORM);

    lightusd_value_t *value = lightusd_value_create_double(2.718);
    lightusd_property_set_value(prop, value);
    lightusd_value_destroy(value);

    lightusd_property_add_target(prop, "/path/to/target1");
    lightusd_property_add_target(prop, "/path/to/target2");

    /* Convert to JSON */
    lightusd_json_value_t *json = lightusd_property_to_json(prop);
    TEST_ASSERT(json != NULL && lightusd_json_value_is_object(json), "Property USD->JSON conversion failed");

    lightusd_json_object_t *obj = lightusd_json_value_get_object(json);
    TEST_ASSERT(lightusd_json_object_has_key(obj, "name"), "JSON should have 'name' key");
    TEST_ASSERT(lightusd_json_object_has_key(obj, "type_name"), "JSON should have 'type_name' key");
    TEST_ASSERT(lightusd_json_object_has_key(obj, "property_type"), "JSON should have 'property_type' key");
    TEST_ASSERT(lightusd_json_object_has_key(obj, "variability"), "JSON should have 'variability' key");
    TEST_ASSERT(lightusd_json_object_has_key(obj, "is_custom"), "JSON should have 'is_custom' key");
    TEST_ASSERT(lightusd_json_object_has_key(obj, "value"), "JSON should have 'value' key");
    TEST_ASSERT(lightusd_json_object_has_key(obj, "targets"), "JSON should have 'targets' key");

    /* Check values */
    lightusd_json_value_t *name_val = lightusd_json_object_get(obj, "name");
    TEST_ASSERT(strcmp(lightusd_json_value_get_string(name_val), "testProp") == 0, "Property name incorrect in JSON");

    lightusd_json_value_t *is_custom_val = lightusd_json_object_get(obj, "is_custom");
    TEST_ASSERT(lightusd_json_value_get_bool(is_custom_val) == 1, "Property is_custom incorrect in JSON");

    lightusd_json_value_t *value_val = lightusd_json_object_get(obj, "value");
    TEST_ASSERT(lightusd_json_value_get_number(value_val) == 2.718, "Property value incorrect in JSON");

    /* Convert back to USD */
    lightusd_property_t *prop_back = lightusd_json_to_property(json);
    ...
    TEST_ASSERT(prop_back->type == LIGHTUSD_PROP_ATTRIB, "Converted property type incorrect");
    TEST_ASSERT(prop_back->variability == LIGHTUSD_VARIABILITY_UNIFORM, "Converted property variability incorrect");
    ...
    lightusd_property_destroy(prop_back);
...
static int test_usd_layer_json_conversion() {
    ...
    /* Set layer metadata */
    lightusd_layer_set_doc(layer, "Test layer for JSON conversion");
    lightusd_layer_set_up_axis(layer, "Y");
    lightusd_layer_set_meters_per_unit(layer, 0.01);

    /* Create a simple prim */
    lightusd_primspec_t *prim = lightusd_primspec_create("TestPrim", "Mesh", LIGHTUSD_SPEC_DEF);
    lightusd_primspec_set_doc(prim, "A test primitive");

    /* Add a property to the prim */
    lightusd_property_t *prop = lightusd_property_create("testAttr", "float", LIGHTUSD_PROP_ATTRIB);
    lightusd_value_t *prop_value = lightusd_value_create_double(1.23);
    lightusd_property_set_value(prop, prop_value);
    lightusd_value_destroy(prop_value);
    lightusd_primspec_add_property(prim, prop);

    /* Add prim to layer */
    lightusd_layer_add_primspec(layer, prim);

    /* Convert to JSON */
    lightusd_json_value_t *json = lightusd_layer_to_json(layer);
    TEST_ASSERT(json != NULL && lightusd_json_value_is_object(json), "Layer USD->JSON conversion failed");

    lightusd_json_object_t *obj = lightusd_json_value_get_object(json);
    TEST_ASSERT(lightusd_json_object_has_key(obj, "name"), "JSON should have 'name' key");
    TEST_ASSERT(lightusd_json_object_has_key(obj, "metadata"), "JSON should have 'metadata' key");
    TEST_ASSERT(lightusd_json_object_has_key(obj, "primspecs"), "JSON should have 'primspecs' key");

    /* Check metadata */
    lightusd_json_value_t *metadata_val = lightusd_json_object_get(obj, "metadata");
    TEST_ASSERT(lightusd_json_value_is_object(metadata_val), "Metadata should be object");
    ...
    TEST_ASSERT(strcmp(lightusd_json_value_get_string(doc_val), "Test layer for JSON conversion") == 0, "Layer doc incorrect in JSON");

    lightusd_json_value_t *up_axis_val = lightusd_json_object_get(metadata_obj, "up_axis");
    TEST_ASSERT(strcmp(lightusd_json_value_get_string(up_axis_val), "Y") == 0, "Layer up_axis incorrect in JSON");

    /* Convert back to USD */
    lightusd_layer_t *layer_back = lightusd_json_to_layer(json);
    ...
    TEST_ASSERT(lightusd_map_size(layer_back->primspecs) == 1, "Converted layer should have 1 primspec");

    lightusd_primspec_t *prim_back = lightusd_layer_get_primspec(layer_back, "TestPrim");
    ...
    lightusd_json_value_destroy(json);
    lightusd_layer_destroy(layer_back);
...
static int test_json_roundtrip_conversion() {
    ...
    lightusd_layer_t *original = lightusd_layer_create("RoundtripTest");
    lightusd_layer_set_doc(original, "Roundtrip test layer");
    lightusd_layer_set_up_axis(original, "Z");
    lightusd_layer_set_meters_per_unit(original, 1.0);

    /* Create root prim */
    lightusd_primspec_t *root = lightusd_primspec_create("World", "Xform", LIGHTUSD_SPEC_DEF);
    lightusd_primspec_set_doc(root, "Root transform");
    ...
    lightusd_primspec_add_property(root, xform_prop);

    /* Create child mesh */
    lightusd_primspec_t *mesh = lightusd_primspec_create("TestMesh", "Mesh", LIGHTUSD_SPEC_DEF);

    /* Add mesh properties */
    lightusd_property_t *points_prop = lightusd_property_create("points", "point3f[]", LIGHTUSD_PROP_ATTRIB);
    lightusd_property_t *material_rel = lightusd_property_create("material:binding", "token", LIGHTUSD_PROP_RELATION);
    lightusd_property_add_target(material_rel, "/World/Materials/TestMaterial");

    lightusd_primspec_add_property(mesh, points_prop);
    lightusd_primspec_add_property(mesh, material_rel);

    /* Build hierarchy */
    lightusd_primspec_add_child(root, mesh);
    lightusd_layer_add_primspec(original, root);

    /* Convert to JSON string */
    char *json_str = lightusd_layer_to_json_string_pretty(original, 2);
    ...
    lightusd_layer_t *restored = lightusd_layer_from_json_string(json_str);
    ...
    TEST_ASSERT(lightusd_map_size(restored->primspecs) == 1, "Restored layer should have 1 root primspec");

    lightusd_primspec_t *restored_root = lightusd_layer_get_primspec(restored, "World");
    TEST_ASSERT(restored_root != NULL, "Restored layer should have World primspec");
    TEST_ASSERT(lightusd_map_size(restored_root->children) == 1, "Restored root should have 1 child");
    TEST_ASSERT(lightusd_map_size(restored_root->properties) == 1, "Restored root should have 1 property");

    lightusd_primspec_t *restored_mesh = lightusd_primspec_get_child(restored_root, "TestMesh");
    TEST_ASSERT(restored_mesh != NULL, "Restored root should have TestMesh child");
    TEST_ASSERT(lightusd_map_size(restored_mesh->properties) == 2, "Restored mesh should have 2 properties");

    lightusd_property_t *restored_material_rel = lightusd_primspec_get_property(restored_mesh, "material:binding");
    ...
    lightusd_layer_destroy(original);
    lightusd_layer_destroy(restored);
    free(json_str);
...
static int test_json_file_io() {
    ...
    /* Create a test layer */
    lightusd_layer_t *layer = lightusd_layer_create("FileIOTest");
    lightusd_layer_set_doc(layer, "File I/O test layer");

    lightusd_primspec_t *prim = lightusd_primspec_create("TestPrim", "Sphere", LIGHTUSD_SPEC_DEF);
    lightusd_property_t *radius_prop = lightusd_property_create("radius", "double", LIGHTUSD_PROP_ATTRIB);
    lightusd_value_t *radius_val = lightusd_value_create_double(2.5);
    lightusd_property_set_value(radius_prop, radius_val);
    lightusd_value_destroy(radius_val);
    lightusd_primspec_add_property(prim, radius_prop);
    lightusd_layer_add_primspec(layer, prim);

    /* Save to file */
    const char *filename = "test_layer.json";
    int save_result = lightusd_layer_save_json_pretty(layer, filename, 2);
    TEST_ASSERT(save_result != 0, "Failed to save layer to JSON file");

    /* Load from file */
    lightusd_layer_t *loaded_layer = lightusd_layer_load_json(filename);
    ...
    lightusd_primspec_t *loaded_prim = lightusd_layer_get_primspec(loaded_layer, "TestPrim");
    TEST_ASSERT(loaded_prim != NULL, "Loaded layer should have TestPrim");

    lightusd_property_t *loaded_radius = lightusd_primspec_get_property(loaded_prim, "radius");
    ...
    double radius_value;
    lightusd_value_get_double(&loaded_radius->value, &radius_value);
    TEST_ASSERT(radius_value == 2.5, "Loaded radius value incorrect");

    /* Clean up */
    lightusd_layer_destroy(layer);
    lightusd_layer_destroy(loaded_layer);
    remove(filename);
...
int main(void) {
    printf("LIGHTUSD JSON Library Test Suite\n");
    printf("============================\n\n");
