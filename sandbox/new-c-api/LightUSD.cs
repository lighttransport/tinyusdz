/// <summary>
/// LightUSD C# P/Invoke Bindings
///
/// C# bindings for the LightUSD C99 API using P/Invoke.
///
/// Usage:
///     LightUSD.Init();
///     var stage = LightUSD.LoadFromFile("model.usd");
...
public class LightUSD
{
    private const string LibraryName = "lightusd_c";
...
class Program
{
    static void Main(string[] args)
    {
        ...
        {
            LightUSD.Init();
            Console.WriteLine($"LightUSD Version: {LightUSD.GetVersion()}");

            if (args.Length > 0)
            {
                using (var stage = LightUSD.LoadFromFile(args[0]))
                {
                    var root = stage.RootPrim;
                    if (root != null)
                    {
