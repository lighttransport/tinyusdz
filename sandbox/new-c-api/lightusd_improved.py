"""
LightUSD Improved Python Bindings

Enhanced, Pythonic bindings for the LightUSD C99 API with:
...
logger = logging.getLogger("lightusd")
...
class LightUSDError(Exception):
    """Base exception for LightUSD errors"""
    pass

class LightUSDLoadError(LightUSDError):
    """Error loading USD file"""
    pass

class LightUSDTypeError(LightUSDError):
    """Wrong type for operation"""
    pass

class LightUSDValueError(LightUSDError):
    """Invalid value"""
    pass

class LightUSDNotFoundError(LightUSDError):
...
def _find_library() -> str:
    ...
    raise LightUSDError(
        "Cannot find liblightusd_c. Install the C library first or set LD_LIBRARY_PATH"
    )
...
class Value:
    ...
    def __init__(self, handle: ctypes.c_void_p):
        if not handle:
            raise LightUSDValueError("Invalid value handle")
        self._handle = handle
...
class Prim:
    ...
    def __init__(self, handle: ctypes.c_void_p, stage: 'Stage' = None):
        if not handle:
            raise LightUSDValueError("Invalid prim handle")
        self._handle = handle
        self._stage = stage
...
class Stage:
    ...
    def __init__(self, handle: ctypes.c_void_p):
        if not handle:
            raise LightUSDLoadError("Invalid stage handle")
        self._handle = handle
...
class LightUSD:
    """Main LightUSD API with context manager support"""
    ...
    def load_file(self, filepath: Union[str, Path], max_memory_mb: int = 0) -> Stage:
        ...
        if result != 0:
            ...
            raise LightUSDLoadError(f"Failed to load '{filepath}': {error_msg}")
    ...
    def load_from_memory(self, data: bytes, format: Format = Format.AUTO) -> Stage:
        ...
        if result != 0:
            error_msg = error_buf.value.decode('utf-8', errors='ignore').strip()
            raise LightUSDLoadError(f"Failed to load from memory: {error_msg}")
    ...
    def __repr__(self) -> str:
        return f"LightUSD(version={self.get_version()})"
