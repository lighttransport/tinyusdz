...
static tusdz_prim_type get_prim_type(const lightusd::Prim& prim) {
    // Check concrete prim types
    if (prim.is<lightusd::Xform>()) return TUSDZ_PRIM_XFORM;
    if (prim.is<lightusd::GeomMesh>()) return TUSDZ_PRIM_MESH;
    if (prim.is<lightusd::Material>()) return TUSDZ_PRIM_MATERIAL;
    if (prim.is<lightusd::Shader>()) return TUSDZ_PRIM_SHADER;
    if (prim.is<lightusd::GeomCamera>()) return TUSDZ_PRIM_CAMERA;
    if (prim.is<lightusd::SphereLight>()) return TUSDZ_PRIM_SPHERE_LIGHT;
    if (prim.is<lightusd::DistantLight>()) return TUSDZ_PRIM_DISTANT_LIGHT;
    if (prim.is<lightusd::DiskLight>()) return TUSDZ_PRIM_DISK_LIGHT;
    if (prim.is<lightusd::RectLight>()) return TUSDZ_PRIM_RECT_LIGHT;
    if (prim.is<lightusd::CylinderLight>()) return TUSDZ_PRIM_CYLINDER_LIGHT;
    if (prim.is<lightusd::DomeLight>()) return TUSDZ_PRIM_DOME_LIGHT;
    if (prim.is<lightusd::Skeleton>()) return TUSDZ_PRIM_SKELETON;
    if (prim.is<lightusd::SkelRoot>()) return TUSDZ_PRIM_SKELROOT;
    if (prim.is<lightusd::SkelAnimation>()) return TUSDZ_PRIM_SKELANIMATION;
    if (prim.is<lightusd::Scope>()) return TUSDZ_PRIM_SCOPE;
    if (prim.is<lightusd::GeomSubset>()) return TUSDZ_PRIM_GEOMSUBSET;
    if (prim.is<lightusd::GeomSphere>()) return TUSDZ_PRIM_SPHERE;
    if (prim.is<lightusd::GeomCube>()) return TUSDZ_PRIM_CUBE;
    if (prim.is<lightusd::GeomCylinder>()) return TUSDZ_PRIM_CYLINDER;
    if (prim.is<lightusd::GeomCapsule>()) return TUSDZ_PRIM_CAPSULE;
    if (prim.is<lightusd::GeomCone>()) return TUSDZ_PRIM_CONE;
    if (prim.is<lightusd::GeomBasisCurves>()) return TUSDZ_PRIM_BASIS_CURVES;
    if (prim.is<lightusd::GeomNurbsCurves>()) return TUSDZ_PRIM_NURBS_CURVE;
    if (prim.is<lightusd::GeomNurbsPatch>()) return TUSDZ_PRIM_NURBS_PATCH;
    if (prim.is<lightusd::GeomPointInstancer>()) return TUSDZ_PRIM_POINT_INSTANCER;

    return TUSDZ_PRIM_UNKNOWN;
}

static tusdz_value_type get_value_type(const lightusd::value::Value& value) {
