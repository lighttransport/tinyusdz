"""
LightUSD Complete Python Bindings

Enhanced Python ctypes bindings for the LightUSD C99 API with complete
function coverage including mesh, transform, material, and animation operations.

Run with: python3 lightusd_complete.py <usd_file>
"""
...
def _find_library():
    """Find the LightUSD C library"""
    names = [
        "lightusd_c", "liblightusd_c", "liblightusd_c.so",
        "liblightusd_c.so.1", "liblightusd_c.dylib", "lightusd_c.dll"
    ]
    ...
    local_paths = [
        Path(__file__).parent / "liblightusd_c.so",
        Path(__file__).parent / "build" / "liblightusd_c.so",
        Path(__file__).parent.parent.parent / "build" / "liblightusd_c.so",
    ]
...
if _lib_path is None:
    raise RuntimeError("Cannot find liblightusd_c")
...
class ValueWrapper:
    ...
    def get_bool(self) -> Optional[bool]:
        value = ctypes.c_int()
        _lib.lightusd_value_get_bool.restype = ctypes.c_int
        if _lib.lightusd_value_get_bool(self._handle, ctypes.byref(value)) == Result.SUCCESS:
    ...
    def get_float2(self) -> Optional[Tuple[float, float]]:
        ...
        _lib.lightusd_value_get_float2.restype = ctypes.c_int
        if _lib.lightusd_value_get_float2(self._handle, values) == Result.SUCCESS:
    ...
    def get_float4(self) -> Optional[Tuple[float, float, float, float]]:
        ...
        _lib.lightusd_value_get_float4.restype = ctypes.c_int
        if _lib.lightusd_value_get_float4(self._handle, values) == Result.SUCCESS:
            return tuple(float(v) for v in values)
        return None

    def get_matrix4d(self) -> Optional[np.ndarray]:
        values = (ctypes.c_double * 16)()
        _lib.lightusd_value_get_matrix4d.restype = ctypes.c_int
        if _lib.lightusd_value_get_matrix4d(self._handle, values) == Result.SUCCESS:
            return np.array(values, dtype=np.float64).reshape(4, 4)
        return None

    def is_animated(self) -> bool:
        _lib.lightusd_value_is_animated.restype = ctypes.c_int
        return bool(_lib.lightusd_value_is_animated(self._handle))
    ...
    def get_time_samples(self) -> Optional[Tuple[List[float], int]]:
        ...
        _lib.lightusd_value_get_time_samples.restype = ctypes.c_int
        if _lib.lightusd_value_get_time_samples(self._handle, ctypes.byref(times_ptr), ctypes.byref(count)) == Result.SUCCESS:
    ...
    def __del__(self):
        if self._handle:
            _lib.lightusd_value_free(self._handle)
...
class PrimWrapper:
    ...
    def get_mesh_data(self) -> Optional[MeshData]:
        ...
        _lib.lightusd_mesh_get_points.restype = ctypes.c_int
        if _lib.lightusd_mesh_get_points(self._handle, ctypes.byref(points_ptr), ctypes.byref(point_count)) == Result.SUCCESS:
        ...
        _lib.lightusd_mesh_get_face_counts.restype = ctypes.c_int
        if _lib.lightusd_mesh_get_face_counts(self._handle, ctypes.byref(counts_ptr), ctypes.byref(count_count)) == Result.SUCCESS:
        ...
        _lib.lightusd_mesh_get_indices.restype = ctypes.c_int
        if _lib.lightusd_mesh_get_indices(self._handle, ctypes.byref(indices_ptr), ctypes.byref(index_count)) == Result.SUCCESS:
        ...
        _lib.lightusd_mesh_get_normals.restype = ctypes.c_int
        if _lib.lightusd_mesh_get_normals(self._handle, ctypes.byref(normals_ptr), ctypes.byref(normal_count)) == Result.SUCCESS:
        ...
        _lib.lightusd_mesh_get_uvs.restype = ctypes.c_int
        if _lib.lightusd_mesh_get_uvs(self._handle, ctypes.byref(uvs_ptr), ctypes.byref(uv_count), 0) == Result.SUCCESS:
            if uv_count.value > 0:
    ...
    def get_subdivision_scheme(self) -> Optional[str]:
        ...
        _lib.lightusd_mesh_get_subdivision_scheme.restype = ctypes.c_char_p
        scheme = _lib.lightusd_mesh_get_subdivision_scheme(self._handle)
    ...
    def get_local_matrix(self, time: float = 0.0) -> Optional[Transform]:
        ...
        _lib.lightusd_xform_get_local_matrix.restype = ctypes.c_int
        if _lib.lightusd_xform_get_local_matrix(self._handle, time, matrix) == Result.SUCCESS:
    ...
    def get_world_matrix(self, time: float = 0.0) -> Optional[Transform]:
        ...
        matrix = (ctypes.c_double * 16)()
        _lib.lightusd_prim_get_world_matrix.restype = ctypes.c_int
        if _lib.lightusd_prim_get_world_matrix(self._handle, time, matrix) == Result.SUCCESS:
            mat_array = np.array(matrix, dtype=np.float64).reshape(4, 4)
            return Transform(matrix=mat_array)
        return None
    ...
    def get_bound_material(self) -> Optional['PrimWrapper']:
        ...
        _lib.lightusd_prim_get_bound_material.restype = ctypes.c_void_p
        mat = _lib.lightusd_prim_get_bound_material(self._handle)
    ...
    def get_surface_shader(self) -> Optional['PrimWrapper']:
        ...
        _lib.lightusd_material_get_surface_shader.restype = ctypes.c_void_p
        shader = _lib.lightusd_material_get_surface_shader(self._handle)
    ...
    def get_shader_input(self, name: str) -> Optional[ValueWrapper]:
        ...
        _lib.lightusd_shader_get_input.restype = ctypes.c_void_p
        value = _lib.lightusd_shader_get_input(self._handle, name.encode('utf-8'))
        return ValueWrapper(value) if value else None

    def get_shader_type(self) -> Optional[str]:
        """Get shader type ID"""
        _lib.lightusd_shader_get_type_id.restype = ctypes.c_char_p
        type_id = _lib.lightusd_shader_get_type_id(self._handle)
...
class StageWrapper:
    ...
    def get_time_range(self) -> Optional[Tuple[float, float, float]]:
        ...
        if _lib.lightusd_stage_get_time_range(self._handle, ctypes.byref(start), ctypes.byref(end), ctypes.byref(fps)) == Result.SUCCESS:
    ...
    def get_memory_stats(self) -> Tuple[int, int]:
        ...
        _lib.lightusd_get_memory_stats.argtypes = [ctypes.c_void_p, ctypes.POINTER(ctypes.c_size_t), ctypes.POINTER(ctypes.c_size_t)]
        _lib.lightusd_get_memory_stats(self._handle, ctypes.byref(used), ctypes.byref(peak))
