...
Core loading feature(both USDA and USDC) is now working and production-grade(And no seg gault for corrupted USDC/USDZ input)
Flattened scene only(i.e, USDA/USDC generated by using pxrUSD's `usdcat --flatten` or USDZ scene).
Composition features are work-in-progress(support in next major release v0.9.0(Q1/2023 expected) planned)
